...
boost::shared_ptr<boost::log::sinks::asynchronous_sink<boost::log::sinks::text_ostream_backend>>
    sink;
...
[[nodiscard]] std::unique_ptr<deinit_t> init(int min_log_level) {
  if (sink) {
    // Deinitialize the logging system before reinitializing it. This can probably only ever be hit
    // in tests.
    deinit();
  }
