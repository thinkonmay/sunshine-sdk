...
namespace safe {
template <class T> class event_t {
public:
  using status_t = util::optional_t<T>;

  template <class... Args> void raise(Args &&...args) {
  ...
  // pop and view should not be used interchangeably
  template <class Rep, class Period> status_t pop(std::chrono::duration<Rep, Period> delay) {
    std::unique_lock ul{_lock};
  ...
  // pop and view should not be used interchangeably
  template <class Rep, class Period> status_t view(std::chrono::duration<Rep, Period> delay) {
    std::unique_lock ul{_lock};
...
template <class T> class alarm_raw_t {
public:
  using status_t = util::optional_t<T>;
  ...
  template <class Pred> auto wait(Pred &&pred) {
    std::unique_lock ul(_lock);
...
template <class T> using alarm_t = std::shared_ptr<alarm_raw_t<T>>;

template <class T> alarm_t<T> make_alarm() {
...
auto make_shared(F_Construct &&fc, F_Destruct &&fd) {
  return shared_t<T>{std::forward<F_Construct>(fc), std::forward<F_Destruct>(fd)};
}
...
template <class T> class post_t : public T {
public:
  template <class... Args>
...
template <class T> inline auto lock(const std::weak_ptr<void> &wp) {
...
public:
  template <class T> using event_t = std::shared_ptr<post_t<event_t<T>>>;

  template <class T> using queue_t = std::shared_ptr<post_t<queue_t<T>>>;

  template <class T> event_t<T> event(const std::string_view &id) {
  ...
  template <class T> queue_t<T> queue(const std::string_view &id) {
    std::lock_guard lg{mutex};
