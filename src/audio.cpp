...
void encodeThread(safe::mail_t mail, sample_queue_t samples, config_t config, void *channel_data) {
  ...
  opus_t opus{opus_multistream_encoder_create(
      stream->sampleRate, stream->channelCount, stream->streams, stream->coupledStreams,
  ...
  while (auto sample = samples->pop()) {
    ...
    int bytes = opus_multistream_encode(opus.get(), sample->data(), frame_size, std::begin(packet),
                                        packet.size());
    if (bytes < 0) {
...
void capture(safe::mail_t mail, config_t config, void *channel_data) {
  ...
  while (!shutdown_event->peek()) {
    ...
    case platf::capture_e::reinit:
      ...
      mic.reset();
      do {
        mic = control->microphone(stream->mapping, stream->channelCount, stream->sampleRate,
                                  frame_size);
        if (!mic) {
...
int start_audio_control(audio_ctx_t &ctx) {
  auto fg = util::fail_guard([]() { BOOST_LOG(warning) << "There will be no audio"sv; });
