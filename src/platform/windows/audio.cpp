...
DEFINE_PROPERTYKEY(PKEY_Device_DeviceDesc, 0xa45c254e, 0xdf1c, 0x4efd, 0x80, 0x20, 0x67, 0xd1, 0x46,
                   0xa8, 0x50, 0xe0, 2); // DEVPROP_TYPE_STRING
DEFINE_PROPERTYKEY(PKEY_Device_FriendlyName, 0xa45c254e, 0xdf1c, 0x4efd, 0x80, 0x20, 0x67, 0xd1,
                   0x46, 0xa8, 0x50, 0xe0, 14); // DEVPROP_TYPE_STRING
DEFINE_PROPERTYKEY(PKEY_DeviceInterface_FriendlyName, 0x026e516e, 0xb814, 0x414b, 0x83, 0xcd, 0x85,
                   0x6d, 0x6f, 0xef, 0x48, 0x22, 2);
...
constexpr auto STEAM_AUDIO_DRIVER_PATH =
    L"%CommonProgramFiles(x86)%"
    L"\\Steam\\drivers\\Windows10\\" STEAM_DRIVER_SUBDIR L"\\SteamStreamingSpeakers.inf";

template <class T> void Release(T *p) {
  p->Release();
}

template <class T> void co_task_free(T *p) {
...
int set_wave_format(audio::wave_format_t &wave_format, const format_t &format) {
  ...
  default:
    BOOST_LOG(error) << "Unsupported Wave Format: [0x"sv
                     << util::hex(wave_format->wFormatTag).to_string_view() << ']';
    return -1;
  };
...
audio_client_t make_audio_client(device_t &device, const format_t &format) {
  ...
  auto status = device->Activate(IID_IAudioClient, CLSCTX_ALL, nullptr, (void **)&audio_client);
  ...
  status = audio_client->Initialize(
      ...
      AUDCLNT_STREAMFLAGS_LOOPBACK | AUDCLNT_STREAMFLAGS_EVENTCALLBACK |
          ...
          AUDCLNT_STREAMFLAGS_SRC_DEFAULT_QUALITY, // Enable automatic resampling to 48 KHz
      0, 0, (LPWAVEFORMATEX)&wave_format, nullptr);

  if (status) {
    BOOST_LOG(debug) << "Couldn't initialize audio client for ["sv << format.name << "]: [0x"sv
...
device_t default_device(device_enum_t &device_enum) {
  device_t device;
  HRESULT status;
  status = device_enum->GetDefaultAudioEndpoint(eRender, eConsole, &device);

  if (FAILED(status)) {
    BOOST_LOG(error) << "Couldn't get default audio endpoint [0x"sv
                     << util::hex(status).to_string_view() << ']';
...
public:
  ...
  int init(std::uint32_t sample_rate, std::uint32_t frame_size, std::uint32_t channels_out) {
    ...
    status = CoCreateInstance(CLSID_MMDeviceEnumerator, nullptr, CLSCTX_ALL,
                              IID_IMMDeviceEnumerator, (void **)&device_enum);

    if (FAILED(status)) {
      BOOST_LOG(error) << "Couldn't create Device Enumerator [0x"sv
                       << util::hex(status).to_string_view() << ']';
    ...
    if (FAILED(status)) {
      BOOST_LOG(error) << "Couldn't register endpoint notification [0x"sv
                       << util::hex(status).to_string_view() << ']';
    ...
    for (auto &format : formats) {
      if (format.channels != channels_out) {
        BOOST_LOG(debug) << "Skipping audio format ["sv << format.name << "] with channel count ["sv
                         << format.channels << " != "sv << channels_out << ']';
        continue;
      }
    ...
    if (FAILED(status)) {
      BOOST_LOG(error) << "Couldn't acquire the number of audio frames [0x"sv
                       << util::hex(status).to_string_view() << ']';
...

    {
      ...
      if (!mmcss_task_handle) {
        BOOST_LOG(error) << "Couldn't associate audio capture thread with Pro Audio MMCSS task [0x"
...
private:
  capture_e _fill_buffer() {
    ...
    for (status = audio_capture->GetNextPacketSize(&packet_size);
      ...
      DWORD buffer_flags;
      status = audio_capture->GetBuffer((BYTE **)&sample_aligned.samples,
                                        &block_aligned.audio_sample_size, &buffer_flags, nullptr,
                                        nullptr);
...
public:
  std::optional<sink_t> sink_info() override {
    ...
    auto virtual_device_id = find_device_id_by_name(config::audio.virtual_sink)
                                 .value_or(from_utf8(config::audio.virtual_sink));
    ...
    for (auto x = 0; x < count; ++x) {
      ...
      BOOST_LOG(verbose) << L"===== Device ====="sv << std::endl
                         << L"Device ID          : "sv << wstring.get() << std::endl
                         << L"Device name        : "sv
                         << no_null((LPWSTR)device_friendly_name.prop.pszVal) << std::endl
                         << L"Adapter name       : "sv << adapter_name << std::endl
                         << L"Device description : "sv << no_null((LPWSTR)device_desc.prop.pszVal)
                         << std::endl
                         << std::endl;
   ...
   */
  std::pair<format_t::type_e, std::string_view> get_sink_info(const std::string &sink) {
  ...
  std::unique_ptr<mic_t> microphone(const std::uint8_t *mapping, int channels,
                                    std::uint32_t sample_rate, std::uint32_t frame_size) override {
  ...
  std::optional<std::wstring> set_format(const std::string &sink) {
    ...
    // If the sink isn't a device name, we'll assume it's a device ID
    auto wstring_device_id =
        find_device_id_by_name(sink).value_or(from_utf8(sink_info.second.data()));
    ...
    status =
        policy->SetDeviceFormat(wstring_device_id.c_str(), wave_format.get(), (WAVEFORMATEX *)&p);
    ...
    for (int x = 0; x < (int)ERole_enum_count; ++x) {
      ...
      if (status) {
        ...
        if (status == HRESULT_FROM_WIN32(ERROR_NOT_FOUND) || status == E_INVALIDARG) {
          ...
          BOOST_LOG(warning) << "Couldn't set ["sv << sink << "] to role ["sv << x << "]: 0x"sv
                             << util::hex(status).to_string_view();
        }
   ...
   */
  std::optional<std::wstring> find_device_id_by_name(const std::string &name) {
    if (name.empty()) {
      return std::nullopt;
    }
  ...
  bool install_steam_audio_drivers() {
    ...
    auto fg = util::fail_guard([newdev]() { FreeLibrary(newdev); });

    auto fn_DiInstallDriverW =
        (decltype(DiInstallDriverW) *)GetProcAddress(newdev, "DiInstallDriverW");
    ...
    if (fn_DiInstallDriverW(nullptr, driver_path, 0, nullptr)) {
      ...
      case ERROR_ACCESS_DENIED:
        BOOST_LOG(warning)
            << "Administrator privileges are required to install Steam Streaming Speakers"sv;
        break;
      case ERROR_FILE_NOT_FOUND:
      case ERROR_PATH_NOT_FOUND:
        BOOST_LOG(info)
            << "Steam audio drivers not found. This is expected if you don't have Steam installed."sv;
        break;
      default:
  ...
  int init() {
    auto status = CoCreateInstance(CLSID_CPolicyConfigClient, nullptr, CLSCTX_ALL,
                                   IID_IPolicyConfig, (void **)&policy);

    if (FAILED(status)) {
      BOOST_LOG(error) << "Couldn't create audio policy config: [0x"sv
                       << util::hex(status).to_string_view() << ']';
...
std::unique_ptr<audio_control_t> audio_control() {
  ...
  if (config::audio.install_steam_drivers &&
      !control->find_device_id_by_name("Steam Streaming Speakers"s)) {
