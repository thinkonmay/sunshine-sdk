...
std::string get_mac_address(const std::string_view &address) {
  ...
  for (auto adapter_pos = info.get(); adapter_pos != nullptr; adapter_pos = adapter_pos->Next) {
    for (auto addr_pos = adapter_pos->FirstUnicastAddress; addr_pos != nullptr;
         addr_pos = addr_pos->Next) {
      if (adapter_pos->PhysicalAddressLength != 0 &&
          address == from_sockaddr(addr_pos->Address.lpSockaddr)) {
...
void print_status(const std::string_view &prefix, HRESULT status) {
  ...
  DWORD bytes = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, nullptr,
...
bool IsUserAdmin(HANDLE user_token) {
  ...
  if (ret) {
    if (!CheckTokenMembership(user_token, AdministratorsGroup, &ret)) {
      ret = false;
      BOOST_LOG(error) << "Failed to verify token membership for administrative access: "
                       << GetLastError();
    }
 ...
 * @return The user's token. If user has admin capability it will be elevated, otherwise it will be
 * a limited token. On error, `nullptr`.
 */
HANDLE
retrieve_users_token(bool elevated) {
  ...
  if (0xFFFFFFFF == consoleSessionId) {
    ...
    BOOST_LOG(warning) << "There isn't an active user session, therefore it is not possible to "
                          "execute commands under the users profile.";
    return nullptr;
  }

  // Get the user token for the active console session
  if (!WTSQueryUserToken(consoleSessionId, &userToken)) {
    BOOST_LOG(debug) << "QueryUserToken failed, this would prevent commands from launching under "
                        "the users profile.";
    return nullptr;
  }
  ...
  // The documentation for this scenario is conflicting, so we'll double check to see if user is
  // actually an admin.
  if (elevated && (elevationType == TokenElevationTypeDefault && !IsUserAdmin(userToken))) {
    // We don't have to strip the token or do anything here, but let's give the user a warning so
    // they're aware what is happening.
    BOOST_LOG(warning) << "This command requires elevation and the current user account logged in "
                          "does not have administrator rights. "
                       << "For security reasons Sunshine will retain the same access level as the "
                          "current user and will not elevate it.";
  }
  ...
  if (elevated && elevationType == TokenElevationTypeLimited) {
    ...
    if (!GetTokenInformation(userToken, TokenLinkedToken, reinterpret_cast<void *>(&linkedToken),
  ...
  // We don't need to do anything for TokenElevationTypeFull users here, because they're already
  // elevated.
  return userToken;
}
 ...
 * @return An `std::error_code` object that will store any error that occurred during the
 * impersonation
 */
...
void restart_on_exit() {
  ...
  if (!CreateProcessW(executable, GetCommandLineW(), nullptr, nullptr, false,
...
static BOOL CALLBACK prgrp_enum_windows(HWND hwnd, LPARAM lParam) {
  ...
  if (enum_ctx->process_ids.find(wnd_process_id) != enum_ctx->process_ids.end()) {
    ...
    if (SendNotifyMessageW(hwnd, WM_CLOSE, 0, 0)) {
      ...
      BOOST_LOG(warning) << "Failed to send WM_CLOSE to PID ["sv << wnd_process_id
                         << "]: " << error;
    }
  }
...
bool request_process_group_exit(std::uintptr_t native_handle) {
  ...
  auto fg = util::fail_guard([&process_id_list]() { free(process_id_list); });
  while (
      !(success = QueryInformationJobObject(job_handle, JobObjectBasicProcessIdList,
...
bool process_group_running(std::uintptr_t native_handle) {
  ...
  if (!QueryInformationJobObject((HANDLE)native_handle, JobObjectBasicAccountingInformation,
                                 &accounting_info, sizeof(accounting_info), nullptr)) {
    auto err = GetLastError();
...
bool send_batch(batched_send_info_t &send_info) {
  ...
  char cmbuf[WSA_CMSG_SPACE(sizeof(DWORD)) + std::max(WSA_CMSG_SPACE(sizeof(IN6_PKTINFO)),
  ...
  DWORD bytes_sent;
  return WSASendMsg((SOCKET)send_info.native_socket, &msg, 1, &bytes_sent, nullptr, nullptr) !=
         SOCKET_ERROR;
}
...
bool send(send_info_t &send_info) {
  ...
  DWORD bytes_sent;
  if (WSASendMsg((SOCKET)send_info.native_socket, &msg, 1, &bytes_sent, nullptr, nullptr) ==
      SOCKET_ERROR) {
...
std::unique_ptr<deinit_t> enable_socket_qos(uintptr_t native_socket,
  ...
  std::call_once(load_qwave_once_flag, []() {
    ...
    fn_QOSAddSocketToFlow =
        (decltype(fn_QOSAddSocketToFlow))GetProcAddress(qwave, "QOSAddSocketToFlow");
    fn_QOSRemoveSocketFromFlow =
        (decltype(fn_QOSRemoveSocketFromFlow))GetProcAddress(qwave, "QOSRemoveSocketFromFlow");
  ...
  if (!fn_QOSAddSocketToFlow(qos_handle, (SOCKET)native_socket, dest_addr, traffic_type,
