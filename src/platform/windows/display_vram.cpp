...
static void free_frame(AVFrame *frame) { av_frame_free(&frame); }
...
buf_t make_buffer(device_t::pointer device, const T &t) {
    static_assert(sizeof(T) % 16 == 0,
                  "Buffer needs to be aligned on a 16-byte alignment");

    D3D11_BUFFER_DESC buffer_desc{sizeof(T), D3D11_USAGE_IMMUTABLE,
    ...
    if (status) {
        BOOST_LOG(error) << "Failed to create buffer: [0x"sv
...
blend_t make_blend(device_t::pointer device, bool enable, bool invert) {
    ...
    if (status) {
        BOOST_LOG(error) << "Failed to create blend state: [0x"sv
                         << util::hex(status).to_string_view() << ']';
        return nullptr;
    }
...
struct texture_lock_helper {
    ...
    texture_lock_helper(IDXGIKeyedMutex *mutex) : _mutex(mutex) {
...
util::buffer_t<std::uint8_t> make_cursor_xor_image(
    ...
    switch (shape_info.Type) {
        ...
        case DXGI_OUTDUPL_POINTER_SHAPE_TYPE_MASKED_COLOR: {
            ...
            std::for_each(
                (std::uint32_t *)std::begin(cursor_img),
                (std::uint32_t *)std::end(cursor_img), [](auto &pixel) {
                    auto alpha = (std::uint8_t)((pixel >> 24) & 0xFF);
                    if (alpha == 0xFF) {
                        // Pixels with 0xFF alpha will be XOR-blended as is.
                    } else if (alpha == 0x00) {
                        // Pixels with 0x00 alpha will be blended by
                        // make_cursor_alpha_image(). We make them transparent
                        // for the XOR-blended cursor image.
                        pixel = transparent;
                    } else {
                        // Other alpha values are illegal in masked color
                        // cursors
                        BOOST_LOG(warning)
                            << "Illegal alpha value in masked color cursor: "
                            << alpha;
                    }
                });
            return cursor_img;
        ...
        default:
            BOOST_LOG(error)
                << "Invalid cursor shape type: " << shape_info.Type;
            return {};
    }

    shape_info.Height /= 2;

    util::buffer_t<std::uint8_t> cursor_img{shape_info.Width *
                                            shape_info.Height * 4};
    ...
    for (auto x = 0; x < bytes; ++x) {
        for (auto c = 7; c >= 0; --c) {
            auto bit = 1 << c;
            auto color_type =
                ((*and_mask & bit) ? 1 : 0) + ((*xor_mask & bit) ? 2 : 0);
...
util::buffer_t<std::uint8_t> make_cursor_alpha_image(
    const util::buffer_t<std::uint8_t> &img_data,
    DXGI_OUTDUPL_POINTER_SHAPE_INFO shape_info) {
    ...
    switch (shape_info.Type) {
        case DXGI_OUTDUPL_POINTER_SHAPE_TYPE_MASKED_COLOR: {
            ...
            std::for_each(
                ...
                (std::uint32_t *)std::end(cursor_img), [](auto &pixel) {
                    auto alpha = (std::uint8_t)((pixel >> 24) & 0xFF);
                    if (alpha == 0xFF) {
                        // Pixels with 0xFF alpha will be XOR-blended by
                        // make_cursor_xor_image(). We make them transparent for
                        // the alpha-blended cursor image.
                        pixel = transparent;
                    } else if (alpha == 0x00) {
                        // Pixels with 0x00 alpha will be blended as opaque with
        ...
        case DXGI_OUTDUPL_POINTER_SHAPE_TYPE_COLOR:
            // Color cursors are just an ARGB bitmap which requires no
            // processing.
            return img_data;
        case DXGI_OUTDUPL_POINTER_SHAPE_TYPE_MONOCHROME:
...
blob_t compile_shader(LPCSTR file, LPCSTR entrypoint, LPCSTR shader_model) {
    ...
    auto status = D3DCompileFromFile(
        wFile.c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE, entrypoint,
        shader_model, flags, 0, &compiled_p, &msg_p);

    if (msg_p) {
        BOOST_LOG(warning) << std::string_view{
            (const char *)msg_p->GetBufferPointer(),
            msg_p->GetBufferSize() - 1};
        msg_p->Release();
    }

    if (status) {
        BOOST_LOG(error) << "Couldn't compile ["sv << file << "] [0x"sv
                         << util::hex(status).to_string_view() << ']';
        return nullptr;
    }
...
class d3d_base_encode_device final {
   public:
    int convert(platf::img_t &img_base) {
        // Garbage collect mapped capture images whose weak references have
        // expired
        ...
        device_ctx->PSSetShader(img.format == DXGI_FORMAT_R16G16B16A16_FLOAT
                                    ? convert_Y_fp16_ps.get()
                                    : convert_Y_ps.get(),
                                nullptr, 0);
    ...
    int init_output(ID3D11Texture2D *frame_texture, int width, int height) {
        // The underlying frame pool owns the texture, so we must reference it
        // for ourselves
        ...
        outY_view = D3D11_VIEWPORT{offsetX,      offsetY, out_width_f,
                                   out_height_f, 0.0f,    1.0f};
        outUV_view =
            D3D11_VIEWPORT{offsetX / 2,      offsetY / 2, out_width_f / 2,
                           out_height_f / 2, 0.0f,        1.0f};

        float subsample_offset_in[16 / sizeof(float)]{
            1.0f / (float)out_width_f,
            1.0f / (float)out_height_f};  // aligned to 16-byte
        subsample_offset = make_buffer(device.get(), subsample_offset_in);

        if (!subsample_offset) {
            BOOST_LOG(error)
                << "Failed to create subsample offset vertex constant buffer";
            return -1;
        }
        device_ctx->VSSetConstantBuffers(0, 1, &subsample_offset);

        {
            int32_t rotation_modifier =
                display->display_rotation == DXGI_MODE_ROTATION_UNSPECIFIED
                    ? 0
                    : display->display_rotation - 1;
            int32_t rotation_data[16 / sizeof(int32_t)]{
                -rotation_modifier};  // aligned to 16-byte
            auto rotation = make_buffer(device.get(), rotation_data);
            if (!rotation) {
                BOOST_LOG(error) << "Failed to create display rotation vertex "
                                    "constant buffer";
                return -1;
            }
        ...
        D3D11_RENDER_TARGET_VIEW_DESC nv12_rt_desc{
            format == DXGI_FORMAT_P010 ? DXGI_FORMAT_R16_UNORM
                                       : DXGI_FORMAT_R8_UNORM,
            D3D11_RTV_DIMENSION_TEXTURE2D};

        auto status = device->CreateRenderTargetView(output_texture.get(),
                                                     &nv12_rt_desc, &nv12_Y_rt);
        if (FAILED(status)) {
            BOOST_LOG(error) << "Failed to create render target view [0x"sv
                             << util::hex(status).to_string_view() << ']';
            return -1;
        }

        nv12_rt_desc.Format = (format == DXGI_FORMAT_P010)
                                  ? DXGI_FORMAT_R16G16_UNORM
                                  : DXGI_FORMAT_R8G8_UNORM;

        status = device->CreateRenderTargetView(output_texture.get(),
                                                &nv12_rt_desc, &nv12_UV_rt);
        if (FAILED(status)) {
            BOOST_LOG(error) << "Failed to create render target view [0x"sv
                             << util::hex(status).to_string_view() << ']';
            return -1;
    ...
    int init(std::shared_ptr<platf::display_t> display,
             adapter_t::pointer adapter_p, pix_fmt_e pix_fmt) {
        ...
        HRESULT status =
            D3D11CreateDevice(adapter_p, D3D_DRIVER_TYPE_UNKNOWN, nullptr,
                              D3D11_CREATE_DEVICE_FLAGS, featureLevels,
                              sizeof(featureLevels) / sizeof(D3D_FEATURE_LEVEL),
                              D3D11_SDK_VERSION, &device, nullptr, &device_ctx);

        if (FAILED(status)) {
            BOOST_LOG(error) << "Failed to create encoder D3D11 device [0x"sv
                             << util::hex(status).to_string_view() << ']';
            return -1;
        }
        ...
        if (FAILED(status)) {
            BOOST_LOG(warning)
                << "Failed to increase encoding GPU thread priority. Please "
                   "run application as administrator for optimal performance.";
        }

        format =
            (pix_fmt == pix_fmt_e::nv12 ? DXGI_FORMAT_NV12 : DXGI_FORMAT_P010);
        status = device->CreateVertexShader(
            convert_yuv420_planar_y_vs_hlsl->GetBufferPointer(),
            convert_yuv420_planar_y_vs_hlsl->GetBufferSize(), nullptr,
            &scene_vs);
        if (status) {
            BOOST_LOG(error) << "Failed to create scene vertex shader [0x"sv
                             << util::hex(status).to_string_view() << ']';
            return -1;
        }

        status = device->CreateVertexShader(
            convert_yuv420_packed_uv_type0_vs_hlsl->GetBufferPointer(),
            convert_yuv420_packed_uv_type0_vs_hlsl->GetBufferSize(), nullptr,
            &convert_UV_vs);
        if (status) {
            BOOST_LOG(error) << "Failed to create convertUV vertex shader [0x"sv
                             << util::hex(status).to_string_view() << ']';
            return -1;
        }

        // If the display is in HDR and we're streaming HDR, we'll be converting
        // scRGB to SMPTE 2084 PQ.
        if (format == DXGI_FORMAT_P010 && display->is_hdr()) {
            ...
            if (status) {
                BOOST_LOG(error)
                    << "Failed to create convertY pixel shader [0x"sv
                    << util::hex(status).to_string_view() << ']';
                return -1;
            }

            status = device->CreatePixelShader(
                convert_yuv420_packed_uv_type0_ps_perceptual_quantizer_hlsl
                    ->GetBufferPointer(),
                convert_yuv420_packed_uv_type0_ps_perceptual_quantizer_hlsl
                    ->GetBufferSize(),
                nullptr, &convert_UV_fp16_ps);
            if (status) {
                BOOST_LOG(error)
                    << "Failed to create convertUV pixel shader [0x"sv
                    << util::hex(status).to_string_view() << ']';
                return -1;
            }
        } else {
            // If the display is in Advanced Color mode, the desktop format will
            // be scRGB FP16. scRGB uses linear gamma, so we must use our linear
            // to sRGB conversion shaders.
            status = device->CreatePixelShader(
                convert_yuv420_planar_y_ps_linear_hlsl->GetBufferPointer(),
                convert_yuv420_planar_y_ps_linear_hlsl->GetBufferSize(),
                nullptr, &convert_Y_fp16_ps);
            if (status) {
                BOOST_LOG(error)
                    << "Failed to create convertY pixel shader [0x"sv
            ...
            status = device->CreatePixelShader(
                convert_yuv420_packed_uv_type0_ps_linear_hlsl
                    ->GetBufferPointer(),
                convert_yuv420_packed_uv_type0_ps_linear_hlsl->GetBufferSize(),
                nullptr, &convert_UV_fp16_ps);
            if (status) {
                BOOST_LOG(error)
                    << "Failed to create convertUV pixel shader [0x"sv
        ...
        status = device->CreatePixelShader(
            ...
            convert_yuv420_planar_y_ps_hlsl->GetBufferSize(), nullptr,
            &convert_Y_ps);
        if (status) {
            BOOST_LOG(error) << "Failed to create convertY pixel shader [0x"sv
                             << util::hex(status).to_string_view() << ']';
            return -1;
        }
        ...
        status = device->CreatePixelShader(
            ...
            convert_yuv420_packed_uv_type0_ps_hlsl->GetBufferSize(), nullptr,
            &convert_UV_ps);
        if (status) {
            BOOST_LOG(error) << "Failed to create convertUV pixel shader [0x"sv
                             << util::hex(status).to_string_view() << ']';
            return -1;
        }

        auto default_color_vectors = ::video::color_vectors_from_colorspace(
            ::video::colorspace_e::rec601, false);
        ...
        device_ctx->IASetPrimitiveTopology(
    ...
    int initialize_image_context(const img_d3d_t &img,
                                 encoder_img_ctx_t &img_ctx) {
        // If we've already opened the shared texture, we're done
        if (img_ctx.encoder_texture &&
            img.capture_texture.get() == img_ctx.capture_texture_p) {
            return 0;
        }

        // Reset this image context in case it was used before with a different
        // texture. Textures can change when transitioning from a dummy image to
        // a real image.
        img_ctx.reset();
        ...
        if (FAILED(status)) {
            BOOST_LOG(error) << "Failed to query ID3D11Device1 [0x"sv
                             << util::hex(status).to_string_view() << ']';
            return -1;
        }
        ...
        status = device1->OpenSharedResource1(
            img.encoder_texture_handle, __uuidof(ID3D11Texture2D),
            (void **)&img_ctx.encoder_texture);
        if (FAILED(status)) {
            BOOST_LOG(error) << "Failed to open shared image texture [0x"sv
                             << util::hex(status).to_string_view() << ']';
            return -1;
        }

        // Get the keyed mutex to synchronize with the capture code
        status = img_ctx.encoder_texture->QueryInterface(
            __uuidof(IDXGIKeyedMutex), (void **)&img_ctx.encoder_mutex);
        if (FAILED(status)) {
            BOOST_LOG(error) << "Failed to query IDXGIKeyedMutex [0x"sv
                             << util::hex(status).to_string_view() << ']';
            return -1;
        }
        ...
        status = device->CreateShaderResourceView(
            img_ctx.encoder_texture.get(), nullptr, &img_ctx.encoder_input_res);
        if (FAILED(status)) {
            BOOST_LOG(error)
                << "Failed to create shader resource view for encoding [0x"sv
                << util::hex(status).to_string_view() << ']';
            return -1;
        }
    ...
    // amongst multiple hwdevice_t objects (and therefore multiple
    // ID3D11Devices).
...
class d3d_avcodec_encode_device_t : public avcodec_encode_device_t {
   public:
    ...
    void apply_colorspace() override { base.apply_colorspace(colorspace); }
    ...
    int prepare_to_derive_context(int hw_device_type) override {
        ...
        if (hw_device_type == AV_HWDEVICE_TYPE_QSV) {
            ...
            auto status = base.device->QueryInterface(IID_ID3D11Multithread,
                                                      (void **)&mt);
            if (FAILED(status)) {
                BOOST_LOG(warning)
    ...
    int set_frame(AVFrame *frame, AVBufferRef *hw_frames_ctx) override {
        ...
        if (!frame->buf[0]) {
            ...
            if (err) {
                ...
                BOOST_LOG(error) << "Failed to get hwframe buffer: "sv
        ...
        // If this is a frame from a derived context, we'll need to map it to
        // D3D11
        ...
        if (frame->format != AV_PIX_FMT_D3D11) {
            ...
            auto err =
                av_hwframe_map(d3d11_frame.get(), frame,
                               AV_HWFRAME_MAP_WRITE | AV_HWFRAME_MAP_OVERWRITE);
            if (err) {
                char err_str[AV_ERROR_MAX_STRING_SIZE]{0};
                BOOST_LOG(error) << "Failed to map D3D11 frame: "sv
                                 << av_make_error_string(
                                        err_str, AV_ERROR_MAX_STRING_SIZE, err);
                return -1;
            }
...
class d3d_nvenc_encode_device_t : public nvenc_encode_device_t {
   public:
    bool init_device(std::shared_ptr<platf::display_t> display,
                     adapter_t::pointer adapter_p, pix_fmt_e pix_fmt) {
        buffer_format = nvenc::nvenc_format_from_sunshine_format(pix_fmt);
        if (buffer_format == NV_ENC_BUFFER_FORMAT_UNDEFINED) {
            BOOST_LOG(error) << "Unexpected pixel format for NvENC ["sv
                             << from_pix_fmt(pix_fmt) << ']';
            return false;
        }
    ...
    bool init_encoder(
        ...
        const ::video::sunshine_colorspace_t &colorspace) override {
        if (!nvenc_d3d) return false;

        auto nvenc_colorspace =
            nvenc::nvenc_colorspace_from_sunshine_colorspace(colorspace);
        if (!nvenc_d3d->create_encoder(config::video.nv, client_config,
                                       nvenc_colorspace, buffer_format))
            return false;

        base.apply_colorspace(colorspace);
        return base.init_output(nvenc_d3d->get_input_texture(),
                                client_config.width, client_config.height) == 0;
    }
...
bool set_cursor_texture(device_t::pointer device, gpu_cursor_t &cursor,
    ...
    D3D11_SUBRESOURCE_DATA data{std::begin(cursor_img), 4 * shape_info.Width,
                                0};
    ...
    cursor.input_res.reset();
    status = device->CreateShaderResourceView(texture.get(), nullptr,
                                              &cursor.input_res);
    if (FAILED(status)) {
...
capture_e display_vram_t::snapshot(
    ...
    if (frame_info.PointerShapeBufferSize > 0) {
        ...
        UINT dummy;
        status = dup.dup->GetFramePointerShape(
            img_data.size(), std::begin(img_data), &dummy, &shape_info);
        if (FAILED(status)) {
        ...
        if (!set_cursor_texture(device.get(), cursor_alpha,
                                std::move(alpha_cursor_img), shape_info) ||
            !set_cursor_texture(device.get(), cursor_xor,
                                std::move(xor_cursor_img), shape_info)) {
            return capture_e::error;
        }
    }

    if (frame_info.LastMouseUpdateTime.QuadPart) {
        cursor_alpha.set_pos(frame_info.PointerPosition.Position.x,
                             frame_info.PointerPosition.Position.y, width,
                             height, display_rotation,
                             frame_info.PointerPosition.Visible);

        cursor_xor.set_pos(frame_info.PointerPosition.Position.x,
                           frame_info.PointerPosition.Position.y, width, height,
                           display_rotation,
                           frame_info.PointerPosition.Visible);
    }

    const bool blend_mouse_cursor_flag =
        (cursor_alpha.visible || cursor_xor.visible) && cursor_visible;
    ...
    if (frame_update_flag) {
        ...
        // It's possible for our display enumeration to race with mode changes
        // and result in mismatched image pool and desktop texture sizes. If
        // this happens, just reinit again.
        if (desc.Width != width_before_rotation ||
            desc.Height != height_before_rotation) {
            BOOST_LOG(info)
        ...
        // It's also possible for the capture format to change on the fly. If
        // that happens, reinitialize capture to try format detection again and
        // create new images.
        if (capture_format != desc.Format) {
            BOOST_LOG(info) << "Capture format changed ["sv
    ...
    if (capture_format == DXGI_FORMAT_UNKNOWN) {
        // We don't know the final capture format yet, so we will encode a black
        // dummy image
        ...
        if (src) {
            ...
            if (blend_mouse_cursor_flag) {
                ...
                // Copy the frame to intermediate surface so we can blend this
                // and future mouse cursor updates without new frames from
                // DesktopDuplication. We use direct3d surface directly here and
                // not an image from pull_free_image_cb mainly because it's
                // lighter (surface sharing between direct3d devices produce
                // significant memory overhead).
                last_frame_action = lfa::copy_src_to_surface;
                // Copy the intermediate surface to a new image from
                // pull_free_image_cb and blend the mouse cursor onto it.
                out_frame_action = ofa::copy_last_surface_and_blend_cursor;
            } else {
                // ...and we don't need to blend the mouse cursor.
                // Copy the frame to a new image from pull_free_image_cb and
                // save the shared pointer to the image in case the mouse cursor
                // appears without a new frame from DesktopDuplication.
                last_frame_action = lfa::copy_src_to_img;
                // Use saved last image shared pointer as output image evading
                // copy.
            ...
            if (blend_mouse_cursor_flag) {
                ...
                if (std::holds_alternative<std::shared_ptr<platf::img_t>>(
                        last_frame_variant)) {
                    // We have the shared pointer of the last image, replace it
                    // with intermediate surface while copying contents so we
                    // can blend this and future mouse cursor updates.
                    last_frame_action = lfa::replace_img_with_surface;
                }
                // Copy the intermediate surface which contains last
                // DesktopDuplication frame to a new image from
                // pull_free_image_cb and blend the mouse cursor onto it.
                out_frame_action = ofa::copy_last_surface_and_blend_cursor;
            } else {
                // ...and we don't need to blend the mouse cursor.
                // This happens when the mouse cursor disappears from screen,
                // or there's mouse cursor on screen, but its drawing is
                // disabled in sunshine.
                if (std::holds_alternative<texture2d_t>(last_frame_variant)) {
                    // We have the intermediate surface that was used as the
                    // mouse cursor blending base. Replace it with an image from
                    // pull_free_image_cb copying contents and freeing up the
                    // surface memory. Save the shared pointer to the image in
                    // case the mouse cursor reappears.
    ...
    auto create_surface = [&](texture2d_t &surface) -> bool {
        ...
        if (FAILED(status)) {
            BOOST_LOG(error) << "Failed to create frame copy texture [0x"sv
                             << util::hex(status).to_string_view() << ']';
            return false;
        }
    ...
    auto get_locked_d3d_img = [&](std::shared_ptr<platf::img_t> &img,
                                  bool dummy = false)
        -> std::tuple<std::shared_ptr<img_d3d_t>, texture_lock_helper> {
        auto d3d_img = std::static_pointer_cast<img_d3d_t>(img);

        // Finish creating the image (if it hasn't happened already),
        // also creates synchronization primitives for shared access from
        // multiple direct3d devices.
        if (complete_img(d3d_img.get(), dummy)) return {nullptr, nullptr};

        // This image is shared between capture direct3d device and encoders
        // direct3d devices, we must acquire lock before doing anything to it.
    ...
    switch (last_frame_action) {
        ...
        case lfa::replace_surface_with_img: {
            ...
            if (!p_surface) {
                BOOST_LOG(error)
                    << "Logical error at " << __FILE__ << ":" << __LINE__;
                return capture_e::error;
            }
            ...
            device_ctx->CopyResource(d3d_img->capture_texture.get(),
                                     p_surface->get());

            // We delay the destruction of intermediate surface in case the
            // mouse cursor reappears shortly.
        ...
        case lfa::replace_img_with_surface: {
            auto p_img =
                std::get_if<std::shared_ptr<platf::img_t>>(&last_frame_variant);
            if (!p_img) {
                BOOST_LOG(error)
                    << "Logical error at " << __FILE__ << ":" << __LINE__;
                return capture_e::error;
            ...
            device_ctx->CopyResource(surface.get(),
    ...
    auto blend_cursor = [&](img_d3d_t &d3d_img) {
        ...
        if (cursor_alpha.texture.get()) {
            ...
            device_ctx->OMSetBlendState(blend_alpha.get(), nullptr,
                                        0xFFFFFFFFu);
        ...
        if (cursor_xor.texture.get()) {
            ...
            device_ctx->OMSetBlendState(blend_invert.get(), nullptr,
                                        0x00FFFFFFu);
    ...
    switch (out_frame_action) {
        ...
        case ofa::dummy_fallback: {
            ...
            bool reclear_dummy =
                old_d3d_img->dummy && old_d3d_img->capture_texture;

            auto [d3d_img, lock] = get_locked_d3d_img(img_out, true);
            if (!d3d_img) return capture_e::error;

            if (reclear_dummy) {
                auto dummy_data = std::make_unique<std::uint8_t[]>(
                    d3d_img->row_pitch * d3d_img->height);
                std::fill_n(dummy_data.get(),
                            d3d_img->row_pitch * d3d_img->height, 0);
                device_ctx->UpdateSubresource(d3d_img->capture_texture.get(), 0,
                                              nullptr, dummy_data.get(),
                                              d3d_img->row_pitch, 0);
            }
    ...
    if (old_surface_delayed_destruction &&
        old_surface_timestamp + 10s < std::chrono::steady_clock::now()) {
...
int display_vram_t::init(const ::video::config_t &config,
    ...
    status = device->CreateVertexShader(cursor_vs_hlsl->GetBufferPointer(),
                                        cursor_vs_hlsl->GetBufferSize(),
                                        nullptr, &cursor_vs);
    if (status) {
        BOOST_LOG(error) << "Failed to create scene vertex shader [0x"sv
                         << util::hex(status).to_string_view() << ']';
...

    {
        ...
        if (!rotation) {
            BOOST_LOG(error)
                << "Failed to create display rotation vertex constant buffer";
            return -1;
        }
    ...
    if (config.dynamicRange && is_hdr()) {
        // This shader will normalize scRGB white levels to a user-defined white
        // level
        status = device->CreatePixelShader(
            cursor_ps_normalize_white_hlsl->GetBufferPointer(),
            cursor_ps_normalize_white_hlsl->GetBufferSize(), nullptr,
            &cursor_ps);
        if (status) {
            BOOST_LOG(error)
                << "Failed to create cursor blending (normalized white) pixel shader [0x"sv
                << util::hex(status).to_string_view() << ']';
            return -1;
        }
        ...
        float white_multiplier_data[16 / sizeof(float)]{
            300.0f / 80.f};  // aligned to 16-byte
        auto white_multiplier =
            make_buffer(device.get(), white_multiplier_data);
        if (!white_multiplier) {
            BOOST_LOG(warning)
                << "Failed to create cursor blending (normalized white) white "
                   "multiplier constant buffer";
            return -1;
        }

        device_ctx->PSSetConstantBuffers(1, 1, &white_multiplier);
    } else {
        status = device->CreatePixelShader(cursor_ps_hlsl->GetBufferPointer(),
                                           cursor_ps_hlsl->GetBufferSize(),
                                           nullptr, &cursor_ps);
        if (status) {
            BOOST_LOG(error)
                << "Failed to create cursor blending pixel shader [0x"sv
                << util::hex(status).to_string_view() << ']';
            return -1;
        }
    }
...
// This cannot use ID3D11DeviceContext because it can be called concurrently by
// the encoding thread
int display_vram_t::complete_img(platf::img_t *img_base, bool dummy) {
    auto img = (img_d3d_t *)img_base;

    // If this already has a capture texture and it's not switching dummy state,
    // nothing to do
    ...
    if (!dummy && capture_format == DXGI_FORMAT_UNKNOWN) {
        BOOST_LOG(error) << "display_vram_t::complete_img() called with "
    ...
    img->dummy = dummy;
    img->format = (capture_format == DXGI_FORMAT_UNKNOWN)
    ...
    t.MiscFlags = D3D11_RESOURCE_MISC_SHARED_NTHANDLE |
                  D3D11_RESOURCE_MISC_SHARED_KEYEDMUTEX;

    HRESULT status;
    if (dummy) {
        auto dummy_data =
            std::make_unique<std::uint8_t[]>(img->row_pitch * img->height);
        std::fill_n(dummy_data.get(), img->row_pitch * img->height, 0);
        D3D11_SUBRESOURCE_DATA initial_data{dummy_data.get(),
                                            (UINT)img->row_pitch, 0};
        status =
            device->CreateTexture2D(&t, &initial_data, &img->capture_texture);
    } else {
        status = device->CreateTexture2D(&t, nullptr, &img->capture_texture);
    }
    if (FAILED(status)) {
        BOOST_LOG(error) << "Failed to create img buf texture [0x"sv
                         << util::hex(status).to_string_view() << ']';
        return -1;
    }

    status = device->CreateRenderTargetView(img->capture_texture.get(), nullptr,
                                            &img->capture_rt);
    if (FAILED(status)) {
        BOOST_LOG(error) << "Failed to create render target view [0x"sv
                         << util::hex(status).to_string_view() << ']';
    ...
    status = img->capture_texture->QueryInterface(__uuidof(IDXGIKeyedMutex),
                                                  (void **)&img->capture_mutex);
    if (FAILED(status)) {
        BOOST_LOG(error) << "Failed to query IDXGIKeyedMutex [0x"sv
                         << util::hex(status).to_string_view() << ']';
        return -1;
    ...
    status = img->capture_texture->QueryInterface(__uuidof(IDXGIResource1),
                                                  (void **)&resource);
    if (FAILED(status)) {
        BOOST_LOG(error) << "Failed to query IDXGIResource1 [0x"sv
                         << util::hex(status).to_string_view() << ']';
        return -1;
    }
    ...
    status =
        resource->CreateSharedHandle(nullptr, DXGI_SHARED_RESOURCE_READ,
                                     nullptr, &img->encoder_texture_handle);
    if (FAILED(status)) {
        BOOST_LOG(error) << "Failed to create shared texture handle [0x"sv
                         << util::hex(status).to_string_view() << ']';
        return -1;
    }
...
std::vector<DXGI_FORMAT> display_vram_t::get_supported_capture_formats() {
    return {
        // scRGB FP16 is the ideal format for Wide Color Gamut and Advanced
        // Color
        // displays (both SDR and HDR). This format uses linear gamma, so we
        // will
        // use a linear->PQ shader for HDR and a linear->sRGB shader for SDR.
        DXGI_FORMAT_R16G16B16A16_FLOAT,

        // DXGI_FORMAT_R10G10B10A2_UNORM seems like it might give us frames
        // already
        // converted to SMPTE 2084 PQ, however it seems to actually just clamp
        // the
        // scRGB FP16 values that DWM is using when the desktop format is scRGB
        // FP16.
        //
        // If there is a case where the desktop format is really SMPTE 2084 PQ,
        // it
 ...
 */
bool display_vram_t::is_codec_supported(std::string_view name,
    ...
    if (adapter_desc.VendorId == 0x1002) {  // AMD
        ...
        // Perform AMF version checks if we're using an AMD GPU. This check is
        // placed in display_vram_t to avoid hitting the display_ram_t path
        // which uses software encoding and doesn't touch AMF.
        HMODULE amfrt = LoadLibraryW(AMF_DLL_NAME);
        if (amfrt) {
            auto unload_amfrt =
                util::fail_guard([amfrt]() { FreeLibrary(amfrt); });

            auto fnAMFQueryVersion = (AMFQueryVersion_Fn)GetProcAddress(
                amfrt, AMF_QUERY_VERSION_FUNCTION_NAME);
            if (fnAMFQueryVersion) {
                amf_uint64 version;
                auto result = fnAMFQueryVersion(&version);
                if (result == AMF_OK) {
                    if (config.videoFormat == 2 &&
                        version < AMF_MAKE_FULL_VERSION(1, 4, 30, 0)) {
                        // AMF 1.4.30 adds ultra low latency mode for AV1. Don't
                        // use AV1 on earlier versions. This corresponds to
                        // driver version 23.5.2 (23.10.01.45) or newer.
                        BOOST_LOG(warning)
                            << "AV1 encoding is disabled on AMF version "sv
                            << AMF_GET_MAJOR_VERSION(version) << '.'
                            << AMF_GET_MINOR_VERSION(version) << '.'
                            << AMF_GET_SUBMINOR_VERSION(version) << '.'
                            << AMF_GET_BUILD_VERSION(version);
                        BOOST_LOG(warning)
                            << "If your AMD GPU supports AV1 encoding, update your graphics drivers!"sv;
                        return false;
                    } else if (config.dynamicRange &&
                               version < AMF_MAKE_FULL_VERSION(1, 4, 23, 0)) {
                        // Older versions of the AMD AMF runtime can crash when
                        // fed P010 surfaces. Fail if AMF version is
                        // below 1.4.23 where HEVC Main10 encoding was
                        // introduced. AMF 1.4.23 corresponds to driver
                        // version 21.12.1 (21.40.11.03) or newer.
                        BOOST_LOG(warning)
                            << "HDR encoding is disabled on AMF version "sv
                            << AMF_GET_MAJOR_VERSION(version) << '.'
                            << AMF_GET_MINOR_VERSION(version) << '.'
                            << AMF_GET_SUBMINOR_VERSION(version) << '.'
                            << AMF_GET_BUILD_VERSION(version);
                        BOOST_LOG(warning)
                            << "If your AMD GPU supports HEVC Main10 encoding, update your graphics drivers!"sv;
                        return false;
                    }
                } else {
                    BOOST_LOG(warning)
                        << "AMFQueryVersion() failed: "sv << result;
                }
            } else {
                BOOST_LOG(warning) << "AMF DLL missing export: "sv
        ...
        BOOST_LOG(warning) << "Unknown GPU vendor ID: "
...
display_vram_t::make_avcodec_encode_device(pix_fmt_e pix_fmt) {
    if (pix_fmt != platf::pix_fmt_e::nv12 &&
        pix_fmt != platf::pix_fmt_e::p010) {
        BOOST_LOG(error) << "display_vram_t doesn't support pixel format ["sv
                         << from_pix_fmt(pix_fmt) << ']';
...
#define compile_vertex_shader_helper(x)                                    \
    if (!(x##_hlsl =                                                       \
              compile_vertex_shader(SUNSHINE_SHADERS_DIR "/" #x ".hlsl"))) \
        return -1;
#define compile_pixel_shader_helper(x)                                    \
    if (!(x##_hlsl =                                                      \
              compile_pixel_shader(SUNSHINE_SHADERS_DIR "/" #x ".hlsl"))) \
        return -1;
    ...
    compile_pixel_shader_helper(
        convert_yuv420_packed_uv_type0_ps_perceptual_quantizer);
    ...
    compile_pixel_shader_helper(
        convert_yuv420_planar_y_ps_perceptual_quantizer);
