...
#if !defined(SUNSHINE_SHADERS_DIR) // for testing this needs to be defined in cmake as we don't do
                                   // an install
...
template <class T> buf_t make_buffer(device_t::pointer device, const T &t) {
  static_assert(sizeof(T) % 16 == 0, "Buffer needs to be aligned on a 16-byte alignment");

  D3D11_BUFFER_DESC buffer_desc{sizeof(T), D3D11_USAGE_IMMUTABLE, D3D11_BIND_CONSTANT_BUFFER};
...
util::buffer_t<std::uint8_t> make_cursor_xor_image(const util::buffer_t<std::uint8_t> &img_data,
  ...
  case DXGI_OUTDUPL_POINTER_SHAPE_TYPE_MASKED_COLOR: {
    ...
    std::for_each((std::uint32_t *)std::begin(cursor_img), (std::uint32_t *)std::end(cursor_img),
                  [](auto &pixel) {
...
util::buffer_t<std::uint8_t> make_cursor_alpha_image(const util::buffer_t<std::uint8_t> &img_data,
                                                     DXGI_OUTDUPL_POINTER_SHAPE_INFO shape_info) {
    ...
    std::for_each((std::uint32_t *)std::begin(cursor_img), (std::uint32_t *)std::end(cursor_img),
                  [](auto &pixel) {
                    ...
                    if (alpha == 0xFF) {
                      ...
                      // Pixels with 0x00 alpha will be blended as opaque with the alpha-blended
                      // image.
                      pixel |= 0xFF000000;
                    } else {
                      // Other alpha values are illegal in masked color cursors
...
blob_t compile_shader(LPCSTR file, LPCSTR entrypoint, LPCSTR shader_model) {
  ...
  auto status = D3DCompileFromFile(wFile.c_str(), nullptr, D3D_COMPILE_STANDARD_FILE_INCLUDE,
                                   entrypoint, shader_model, flags, 0, &compiled_p, &msg_p);

  if (msg_p) {
    BOOST_LOG(warning) << std::string_view{(const char *)msg_p->GetBufferPointer(),
...
public:
  int convert(platf::img_t &img_base) {
    ...
    if (!img.blank) {
      ...
      device_ctx->PSSetShader(img.format == DXGI_FORMAT_R16G16B16A16_FLOAT ? convert_Y_fp16_ps.get()
                                                                           : convert_Y_ps.get(),
                              nullptr, 0);
  ...
  int init_output(ID3D11Texture2D *frame_texture, int width, int height) {
    ...
    outUV_view =
        D3D11_VIEWPORT{offsetX / 2, offsetY / 2, out_width_f / 2, out_height_f / 2, 0.0f, 1.0f};

    float subsample_offset_in[16 / sizeof(float)]{1.0f / (float)out_width_f,
    ...
    {
      int32_t rotation_modifier = display->display_rotation == DXGI_MODE_ROTATION_UNSPECIFIED
                                      ? 0
    ...
    D3D11_RENDER_TARGET_VIEW_DESC nv12_rt_desc{format == DXGI_FORMAT_P010 ? DXGI_FORMAT_R16_UNORM
  ...
  int init(std::shared_ptr<platf::display_t> display, adapter_t::pointer adapter_p,
           pix_fmt_e pix_fmt) {
    ...
    HRESULT status =
        D3D11CreateDevice(adapter_p, D3D_DRIVER_TYPE_UNKNOWN, nullptr, D3D11_CREATE_DEVICE_FLAGS,
    ...
    if (FAILED(status)) {
      BOOST_LOG(warning) << "Failed to query DXGI interface from device [0x"sv
                         << util::hex(status).to_string_view() << ']';
      return -1;
    }
    ...
    if (FAILED(status)) {
      BOOST_LOG(warning) << "Failed to increase encoding GPU thread priority. Please run "
                            "application as administrator for optimal performance.";
    }

    format = (pix_fmt == pix_fmt_e::nv12 ? DXGI_FORMAT_NV12 : DXGI_FORMAT_P010);
    status = device->CreateVertexShader(convert_yuv420_planar_y_vs_hlsl->GetBufferPointer(),
    ...
    status = device->CreateVertexShader(convert_yuv420_packed_uv_type0_vs_hlsl->GetBufferPointer(),
                                        convert_yuv420_packed_uv_type0_vs_hlsl->GetBufferSize(),
                                        nullptr, &convert_UV_vs);
    if (status) {
    ...
    if (format == DXGI_FORMAT_P010 && display->is_hdr()) {
      status = device->CreatePixelShader(
          convert_yuv420_planar_y_ps_perceptual_quantizer_hlsl->GetBufferPointer(),
          convert_yuv420_planar_y_ps_perceptual_quantizer_hlsl->GetBufferSize(), nullptr,
          &convert_Y_fp16_ps);
      if (status) {
      ...
      status = device->CreatePixelShader(
          convert_yuv420_packed_uv_type0_ps_perceptual_quantizer_hlsl->GetBufferPointer(),
          convert_yuv420_packed_uv_type0_ps_perceptual_quantizer_hlsl->GetBufferSize(), nullptr,
          &convert_UV_fp16_ps);
      if (status) {
      ...
      status = device->CreatePixelShader(convert_yuv420_planar_y_ps_linear_hlsl->GetBufferPointer(),
      ...
      status = device->CreatePixelShader(
          convert_yuv420_packed_uv_type0_ps_linear_hlsl->GetBufferPointer(),
          convert_yuv420_packed_uv_type0_ps_linear_hlsl->GetBufferSize(), nullptr,
          &convert_UV_fp16_ps);
      if (status) {
        BOOST_LOG(error) << "Failed to create convertUV pixel shader [0x"sv
                         << util::hex(status).to_string_view() << ']';
    ...
    status = device->CreatePixelShader(convert_yuv420_planar_y_ps_hlsl->GetBufferPointer(),
    ...
    status = device->CreatePixelShader(convert_yuv420_packed_uv_type0_ps_hlsl->GetBufferPointer(),
                                       convert_yuv420_packed_uv_type0_ps_hlsl->GetBufferSize(),
                                       nullptr, &convert_UV_ps);
    if (status) {
      BOOST_LOG(error) << "Failed to create convertUV pixel shader [0x"sv
                       << util::hex(status).to_string_view() << ']';
      return -1;
    ...
    auto default_color_vectors =
        ::video::color_vectors_from_colorspace(::video::colorspace_e::rec601, false);
  ...
  int initialize_image_context(const img_d3d_t &img, encoder_img_ctx_t &img_ctx) {
    ...
    status = device1->OpenSharedResource1(img.encoder_texture_handle, __uuidof(ID3D11Texture2D),
    ...
    status = img_ctx.encoder_texture->QueryInterface(__uuidof(IDXGIKeyedMutex),
                                                     (void **)&img_ctx.encoder_mutex);
    if (FAILED(status)) {
    ...
    status = device->CreateShaderResourceView(img_ctx.encoder_texture.get(), nullptr,
                                              &img_ctx.encoder_input_res);
    if (FAILED(status)) {
      BOOST_LOG(error) << "Failed to create shader resource view for encoding [0x"sv
                       << util::hex(status).to_string_view() << ']';
      return -1;
    }
...
public:
  ...
  int set_frame(AVFrame *frame, AVBufferRef *hw_frames_ctx) override {
    ...
    if (frame->format != AV_PIX_FMT_D3D11) {
      ...
      auto err =
          av_hwframe_map(d3d11_frame.get(), frame, AV_HWFRAME_MAP_WRITE | AV_HWFRAME_MAP_OVERWRITE);
      if (err) {
        char err_str[AV_ERROR_MAX_STRING_SIZE]{0};
...
public:
  ...
  bool init_encoder(const ::video::config_t &client_config,
    ...
    if (!nvenc_d3d->create_encoder(config::video.nv, client_config, nvenc_colorspace,
                                   buffer_format))
      return false;

    base.apply_colorspace(colorspace);
    return base.init_output(nvenc_d3d->get_input_texture(), client_config.width,
...
bool set_cursor_texture(device_t::pointer device, gpu_cursor_t &cursor,
  ...
  D3D11_SUBRESOURCE_DATA data{std::begin(cursor_img), 4 * shape_info.Width, 0};
...
capture_e display_ddup_vram_t::snapshot(const pull_free_image_cb_t &pull_free_image_cb,
  ...
  if (auto qpc_displayed =
          std::max(frame_info.LastPresentTime.QuadPart, frame_info.LastMouseUpdateTime.QuadPart)) {
    // Translate QueryPerformanceCounter() value to steady_clock time point
    frame_timestamp =
        std::chrono::steady_clock::now() - qpc_time_difference(qpc_counter(), qpc_displayed);
  }
  ...
  if (frame_info.PointerShapeBufferSize > 0) {
    ...
    UINT dummy;
    status =
        dup.dup->GetFramePointerShape(img_data.size(), std::begin(img_data), &dummy, &shape_info);
    if (FAILED(status)) {
  ...
  if (frame_info.LastMouseUpdateTime.QuadPart) {
    cursor_alpha.set_pos(frame_info.PointerPosition.Position.x,
                         frame_info.PointerPosition.Position.y, width, height, display_rotation,
  ...
  if (frame_update_flag) {
    ...
    if (desc.Width != width_before_rotation || desc.Height != height_before_rotation) {
      BOOST_LOG(info) << "Capture size changed ["sv << width << 'x' << height << " -> "sv
  ...
  if (capture_format == DXGI_FORMAT_UNKNOWN) {
    ...
    if (src) {
      ...
      if (blend_mouse_cursor_flag) {
        ...
        // Copy the frame to intermediate surface so we can blend this and future mouse cursor
        // updates without new frames from DesktopDuplication. We use direct3d surface directly here
        // and not an image from pull_free_image_cb mainly because it's lighter (surface sharing
        // between direct3d devices produce significant memory overhead).
        last_frame_action = lfa::copy_src_to_surface;
        // Copy the intermediate surface to a new image from pull_free_image_cb and blend the mouse
        // cursor onto it.
        out_frame_action = ofa::copy_last_surface_and_blend_cursor;
      } else {
        // ...and we don't need to blend the mouse cursor.
        // Copy the frame to a new image from pull_free_image_cb and save the shared pointer to the
        // image in case the mouse cursor appears without a new frame from DesktopDuplication.
      ...
      if (blend_mouse_cursor_flag) {
        ...
        if (std::holds_alternative<texture2d_t>(last_frame_variant)) {
          ...
          // Replace it with an image from pull_free_image_cb copying contents and freeing up the
          // surface memory. Save the shared pointer to the image in case the mouse cursor
          // reappears.
  ...
  auto get_locked_d3d_img =
      [&](std::shared_ptr<platf::img_t> &img,
          bool dummy = false) -> std::tuple<std::shared_ptr<img_d3d_t>, texture_lock_helper> {
...
int display_ddup_vram_t::init(const ::video::config_t &config, const std::string &display_name) {
  ...
  status = device->CreateVertexShader(cursor_vs_hlsl->GetBufferPointer(),
                                      cursor_vs_hlsl->GetBufferSize(), nullptr, &cursor_vs);
  if (status) {
    BOOST_LOG(error) << "Failed to create scene vertex shader [0x"sv
                     << util::hex(status).to_string_view() << ']';
    return -1;
...

  {
    int32_t rotation_modifier =
        display_rotation == DXGI_MODE_ROTATION_UNSPECIFIED ? 0 : display_rotation - 1;
    int32_t rotation_data[16 / sizeof(int32_t)]{rotation_modifier}; // aligned to 16-byte
    auto rotation = make_buffer(device.get(), rotation_data);
    if (!rotation) {
      BOOST_LOG(error) << "Failed to create display rotation vertex constant buffer";
  ...
  if (config.dynamicRange && is_hdr()) {
    ...
    status = device->CreatePixelShader(cursor_ps_normalize_white_hlsl->GetBufferPointer(),
                                       cursor_ps_normalize_white_hlsl->GetBufferSize(), nullptr,
                                       &cursor_ps);
    if (status) {
      BOOST_LOG(error) << "Failed to create cursor blending (normalized white) pixel shader [0x"sv
                       << util::hex(status).to_string_view() << ']';
      return -1;
    }
    ...
    if (!white_multiplier) {
      BOOST_LOG(warning)
          << "Failed to create cursor blending (normalized white) white multiplier constant buffer";
      return -1;
    }
    ...
    status = device->CreatePixelShader(cursor_ps_hlsl->GetBufferPointer(),
                                       cursor_ps_hlsl->GetBufferSize(), nullptr, &cursor_ps);
    if (status) {
      BOOST_LOG(error) << "Failed to create cursor blending pixel shader [0x"sv
                       << util::hex(status).to_string_view() << ']';
      return -1;
    }
  }
 ...
 */
capture_e display_wgc_vram_t::snapshot(const pull_free_image_cb_t &pull_free_image_cb,
                                       std::shared_ptr<platf::img_t> &img_out,
                                       std::chrono::milliseconds timeout, bool cursor_visible) {
  texture2d_t src;
  uint64_t frame_qpc;
  ...
  auto frame_timestamp =
      std::chrono::steady_clock::now() - qpc_time_difference(qpc_counter(), frame_qpc);
  D3D11_TEXTURE2D_DESC desc;
  src->GetDesc(&desc);
...
int display_vram_t::complete_img(platf::img_t *img_base, bool dummy) {
  ...
  img->dummy = dummy;
  img->format =
      (capture_format == DXGI_FORMAT_UNKNOWN) ? DXGI_FORMAT_B8G8R8A8_UNORM : capture_format;
  ...
  status =
      img->capture_texture->QueryInterface(__uuidof(IDXGIKeyedMutex), (void **)&img->capture_mutex);
  if (FAILED(status)) {
    BOOST_LOG(error) << "Failed to query IDXGIKeyedMutex [0x"sv
                     << util::hex(status).to_string_view() << ']';
    return -1;
  ...
  status = resource->CreateSharedHandle(nullptr, DXGI_SHARED_RESOURCE_READ, nullptr,
                                        &img->encoder_texture_handle);
  if (FAILED(status)) {
...
bool display_vram_t::is_codec_supported(std::string_view name, const ::video::config_t &config) {
  ...
  if (adapter_desc.VendorId == 0x1002) { // AMD
    ...
    if (amfrt) {
      auto unload_amfrt = util::fail_guard([amfrt]() { FreeLibrary(amfrt); });

      auto fnAMFQueryVersion =
          (AMFQueryVersion_Fn)GetProcAddress(amfrt, AMF_QUERY_VERSION_FUNCTION_NAME);
