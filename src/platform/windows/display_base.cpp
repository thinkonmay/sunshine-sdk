...
capture_e duplication_t::next_frame(DXGI_OUTDUPL_FRAME_INFO &frame_info,
    ...
    switch (status) {
        case S_OK:
            // ProtectedContentMaskedOut seems to semi-randomly be TRUE or FALSE
            // even when protected content is on screen the whole time, so we
            // can't just print when it changes. Instead we'll keep track of the
            // last time we printed the warning and print another if we haven't
            // printed one recently.
            if (frame_info.ProtectedContentMaskedOut &&
                std::chrono::steady_clock::now() >
                    last_protected_content_warning_time + 10s) {
                BOOST_LOG(warning)
                    << "Windows is currently blocking DRM-protected content from capture. You may see black regions where this content would be."sv;
                last_protected_content_warning_time =
                    std::chrono::steady_clock::now();
            }
        ...
        default:
            BOOST_LOG(error) << "Couldn't acquire next frame [0x"sv
...
capture_e duplication_t::release_frame() {
    ...
    switch (status) {
        ...
        default:
            BOOST_LOG(error) << "Error while releasing duplication frame [0x"sv
                             << util::hex(status).to_string_view();
            return capture_e::error;
    }
}

duplication_t::~duplication_t() { release_frame(); }

void display_base_t::high_precision_sleep(std::chrono::nanoseconds duration) {
    if (!timer) {
        BOOST_LOG(error)
            << "Attempting high_precision_sleep() with uninitialized timer";
        return;
    }
    if (duration < 0s) {
        BOOST_LOG(error)
            << "Attempting high_precision_sleep() with negative duration";
        return;
    }
    if (duration > 5s) {
        BOOST_LOG(error) << "Attempting high_precision_sleep() with "
                            "unexpectedly large duration (>5s)";
        return;
    }
...
capture_e display_base_t::capture(
    ...
    // However, worst case it will trigger us to reinit DD, waking the display
    // back up in a neverending cycle of waking and sleeping the display of an
    // idle machine.
    ...
    while (true) {
        ...
        // state of the display subsystem. It is recommended to call this once
        // per frame.
...
bool set_gpu_preference_on_self(int preference) {
    ...
    auto status = RegSetKeyValueW(
        HKEY_CURRENT_USER, L"Software\\Microsoft\\DirectX\\UserGpuPreferences",
        sunshine_path, REG_SZ, value_data,
...
// that device to avoid confusing applications. While this works properly for
// most applications, it breaks the Desktop Duplication API because DXGI doesn't
// proxy the virtual DXGIOutput to the real GPU it is attached to. When trying
// to call DuplicateOutput() on one of these virtual outputs, it fails with
// DXGI_ERROR_UNSUPPORTED (even if you try sneaky stuff like passing the
// ID3D11Device for the iGPU and the virtual DXGIOutput from the dGPU). Because
// the GPU preference is once-per-process, we spawn a helper tool to probe for
// us before we set our own GPU preference.
bool probe_for_gpu_preference(const std::string &display_name) {
    ...
    // the GPU driver control panel options. Since ddprobe.exe can have
    // different GPU driver overrides than Sunshine.exe, we want to avoid a
    // scenario where autoselection might work for ddprobe.exe but not for us.
    for (int i = 1; i < 5; i++) {
        ...
        HRESULT result;
        try {
            result = bp::system(cmd, std::to_string(i), display_name,
        ...
        BOOST_LOG(info) << "ddprobe.exe ["sv << i << "] ["sv << display_name
                        << "] returned: 0x"sv
                        << util::hex(result).to_string_view();

        // E_ACCESSDENIED can happen at the login screen. If we get this error,
        // we know capture would have been supported, because
        // DXGI_ERROR_UNSUPPORTED would have been raised first if it wasn't.
...
bool test_dxgi_duplication(adapter_t &adapter, output_t &output) {
    ...
    if (FAILED(status)) {
        BOOST_LOG(error) << "Failed to create D3D11 device for DD test [0x"sv
                         << util::hex(status).to_string_view() << ']';
        return false;
    }
    ...
    BOOST_LOG(error) << "DuplicateOutput() test failed [0x"sv
                     << util::hex(status).to_string_view() << ']';
    return false;
}

int display_base_t::init(const ::video::config_t &config,
    ...
    std::call_once(windows_cpp_once_flag, []() {
        ...
        typedef BOOL (*User32_SetProcessDpiAwarenessContext)(
            DPI_AWARENESS_CONTEXT value);

        auto user32 = LoadLibraryA("user32.dll");
        auto f = (User32_SetProcessDpiAwarenessContext)GetProcAddress(
    ...
    if (!probe_for_gpu_preference(display_name)) {
        BOOST_LOG(warning)
            << "Failed to set GPU preference. Capture may not work!"sv;
    }

    status = CreateDXGIFactory1(IID_IDXGIFactory1, (void **)&factory);
    if (FAILED(status)) {
        BOOST_LOG(error) << "Failed to create DXGIFactory1 [0x"sv
                         << util::hex(status).to_string_view() << ']';
        return -1;
    }
    ...
    for (int tries = 0; tries < 2; ++tries) {
        for (int x = 0;
             factory->EnumAdapters1(x, &adapter_p) != DXGI_ERROR_NOT_FOUND;
             ++x) {
            ...
            if (!adapter_name.empty() &&
                adapter_desc.Description != adapter_name) {
                continue;
            }

            dxgi::output_t::pointer output_p;
            for (int y = 0;
                 adapter_tmp->EnumOutputs(y, &output_p) != DXGI_ERROR_NOT_FOUND;
                 ++y) {
                ...
                if (desc.AttachedToDesktop &&
                    test_dxgi_duplication(adapter_tmp, output_tmp)) {
                    ...
                    // left and bottom may be negative, yet absolute mouse
                    // coordinates start at 0x0 Ensure offset starts at 0x0
        ...
        // If we made it here without finding an output, try to power on the
        // display and retry.
        if (tries == 0) {
    ...
    status = D3D11CreateDevice(
        adapter_p, D3D_DRIVER_TYPE_UNKNOWN, nullptr, D3D11_CREATE_DEVICE_FLAGS,
        featureLevels, sizeof(featureLevels) / sizeof(D3D_FEATURE_LEVEL),
        D3D11_SDK_VERSION, &device, &feature_level, &device_ctx);

    adapter_p->Release();

    if (FAILED(status)) {
        BOOST_LOG(error) << "Failed to create D3D11 device [0x"sv
                         << util::hex(status).to_string_view() << ']';
    ...
    BOOST_LOG(info)
        ...
        << "Device Vendor ID   : 0x"sv
        << util::hex(adapter_desc.VendorId).to_string_view() << std::endl
        << "Device Device ID   : 0x"sv
        << util::hex(adapter_desc.DeviceId).to_string_view() << std::endl
        << "Device Video Mem   : "sv
        << adapter_desc.DedicatedVideoMemory / 1048576 << " MiB"sv << std::endl
        << "Device Sys Mem     : "sv
        << adapter_desc.DedicatedSystemMemory / 1048576 << " MiB"sv << std::endl
        << "Share Sys Mem      : "sv
        << adapter_desc.SharedSystemMemory / 1048576 << " MiB"sv << std::endl
        << "Feature Level      : 0x"sv
        << util::hex(feature_level).to_string_view() << std::endl
    ...
    // Bump up thread priority
    {
        ...
        if (OpenProcessToken(GetCurrentProcess(), flags, &token) &&
            ...
            if (!AdjustTokenPrivileges(token, false, &tp, sizeof(tp), NULL,
                                       NULL)) {
                BOOST_LOG(warning)
                    << "Could not set privilege to increase GPU priority";
            }
        }
        ...
        if (gdi32) {
            auto check_hags = [&](const LUID &adapter) -> bool {
                auto d3dkmt_open_adapter =
                    (PD3DKMTOpenAdapterFromLuid)GetProcAddress(
                        gdi32, "D3DKMTOpenAdapterFromLuid");
                auto d3dkmt_query_adapter_info =
                    (PD3DKMTQueryAdapterInfo)GetProcAddress(
                        gdi32, "D3DKMTQueryAdapterInfo");
                auto d3dkmt_close_adapter = (PD3DKMTCloseAdapter)GetProcAddress(
                    gdi32, "D3DKMTCloseAdapter");
                if (!d3dkmt_open_adapter || !d3dkmt_query_adapter_info ||
                    !d3dkmt_close_adapter) {
                    BOOST_LOG(error)
                        << "Couldn't load d3dkmt functions from gdi32.dll to "
                           "determine GPU HAGS status";
                    return false;
                }
                ...
                if (FAILED(d3dkmt_open_adapter(&d3dkmt_adapter))) {
                    BOOST_LOG(error)
                        << "D3DKMTOpenAdapterFromLuid() failed while trying to "
                           "determine GPU HAGS status";
                    return false;
                }
                ...
                d3dkmt_adapter_info.PrivateDriverDataSize =
                    sizeof(d3dkmt_adapter_caps);

                if (SUCCEEDED(
                        d3dkmt_query_adapter_info(&d3dkmt_adapter_info))) {
                    result = d3dkmt_adapter_caps.HwSchEnabled;
                } else {
                    BOOST_LOG(warning)
                        << "D3DKMTQueryAdapterInfo() failed while trying to "
                           "determine GPU HAGS status";
                    result = false;
                }

                D3DKMT_CLOSEADAPTER d3dkmt_close_adapter_wrap = {
                    d3dkmt_adapter.hAdapter};
                if (FAILED(d3dkmt_close_adapter(&d3dkmt_close_adapter_wrap))) {
                    BOOST_LOG(error) << "D3DKMTCloseAdapter() failed while "
                                        "trying to determine GPU HAGS status";
                }

                return result;
            };

            auto d3dkmt_set_process_priority =
                (PD3DKMTSetProcessSchedulingPriorityClass)GetProcAddress(
            ...
            if (d3dkmt_set_process_priority) {
                ...
                if (adapter_desc.VendorId == 0x10DE) {
                    // As of 2023.07, NVIDIA driver has unfixed bug(s) where
                    // "realtime" can cause unrecoverable encoding freeze or
                    // outright driver crash This issue happens more frequently
                    // with HAGS, in DX12 games or when VRAM is filled close to
                    // max capacity Track OBS to see if they find better
                    // workaround or NVIDIA fixes it on their end, they seem to
                    // be in communication
                    if (hags_enabled && !config::video.nv_realtime_hags)
                        priority = D3DKMT_SCHEDULINGPRIORITYCLASS_HIGH;
                }
                BOOST_LOG(info) << "Active GPU has HAGS "
                                << (hags_enabled ? "enabled" : "disabled");
                BOOST_LOG(info)
                    << "Using "
                    << (priority == D3DKMT_SCHEDULINGPRIORITYCLASS_HIGH
                            ? "high"
                            : "realtime")
                    << " GPU priority";
                if (FAILED(d3dkmt_set_process_priority(GetCurrentProcess(),
                                                       priority))) {
                    BOOST_LOG(warning)
                        << "Failed to adjust GPU priority. Please run "
                           "application as administrator for optimal "
                           "performance.";
                }
            } else {
                BOOST_LOG(error)
                    << "Couldn't load D3DKMTSetProcessSchedulingPriorityClass "
                       "function from gdi32.dll to adjust GPU priority";
            }
        }
        ...
        if (FAILED(status)) {
            BOOST_LOG(warning)
                << "Failed to increase capture GPU thread priority. Please run "
    ...
    // Try to reduce latency
    {
        ...
        if (FAILED(status)) {
            BOOST_LOG(warning) << "Failed to set maximum frame latency [0x"sv
                               << util::hex(status).to_string_view() << ']';
        }
    }
    ...
    {
        // IDXGIOutput5 is optional, but can provide improved performance and
        // wide color support
        ...
        if (SUCCEEDED(status)) {
            ...
            if (supported_formats.empty()) {
                BOOST_LOG(warning)
                    << "No compatible capture formats for this encoder"sv;
                return -1;
            }

            // We try this twice, in case we still get an error on
            // reinitialization
            for (int x = 0; x < 2; ++x) {
                status = output5->DuplicateOutput1(
                    (IUnknown *)device.get(), 0, supported_formats.size(),
                    supported_formats.data(), &dup.dup);
                if (SUCCEEDED(status)) {
                    break;
                }
            ...
            // We don't retry with DuplicateOutput() because we can hit this
            // codepath when we're racing with mode changes and we don't want to
            // accidentally fall back to suboptimal capture if we get unlucky
            // and succeed below.
            if (FAILED(status)) {
                BOOST_LOG(warning) << "DuplicateOutput1 Failed [0x"sv
                                   << util::hex(status).to_string_view() << ']';
                return -1;
            }
        } else {
            BOOST_LOG(warning)
                << "IDXGIOutput5 is not supported by your OS. Capture performance may be reduced."sv;

            dxgi::output1_t output1{};
            status =
                output->QueryInterface(IID_IDXGIOutput1, (void **)&output1);
            if (FAILED(status)) {
                BOOST_LOG(error)
                    << "Failed to query IDXGIOutput1 from the output"sv;
                return -1;
            }

            for (int x = 0; x < 2; ++x) {
                status = output1->DuplicateOutput((IUnknown *)device.get(),
                                                  &dup.dup);
                if (SUCCEEDED(status)) {
                    break;
                }
            ...
            if (FAILED(status)) {
                BOOST_LOG(error) << "DuplicateOutput Failed [0x"sv
                                 << util::hex(status).to_string_view() << ']';
                return -1;
            }
        }
    ...
    BOOST_LOG(info) << "Desktop resolution ["sv << dup_desc.ModeDesc.Width
                    << 'x' << dup_desc.ModeDesc.Height << ']';
    BOOST_LOG(info) << "Desktop format ["sv
                    << dxgi_format_to_string(dup_desc.ModeDesc.Format) << ']';

    display_refresh_rate = dup_desc.ModeDesc.RefreshRate;
    double display_refresh_rate_decimal =
        (double)display_refresh_rate.Numerator /
        display_refresh_rate.Denominator;
    BOOST_LOG(info) << "Display refresh rate [" << display_refresh_rate_decimal
                    << "Hz]";
    ...
    if (SUCCEEDED(status)) {
        ...
        BOOST_LOG(info) << std::endl
                        << "Colorspace         : "sv
                        << colorspace_to_string(desc1.ColorSpace) << std::endl
                        << "Bits Per Color     : "sv << desc1.BitsPerColor
                        << std::endl
                        << "Red Primary        : ["sv << desc1.RedPrimary[0]
                        << ',' << desc1.RedPrimary[1] << ']' << std::endl
                        << "Green Primary      : ["sv << desc1.GreenPrimary[0]
                        << ',' << desc1.GreenPrimary[1] << ']' << std::endl
                        << "Blue Primary       : ["sv << desc1.BluePrimary[0]
                        << ',' << desc1.BluePrimary[1] << ']' << std::endl
                        << "White Point        : ["sv << desc1.WhitePoint[0]
                        << ',' << desc1.WhitePoint[1] << ']' << std::endl
                        << "Min Luminance      : "sv << desc1.MinLuminance
                        << " nits"sv << std::endl
                        << "Max Luminance      : "sv << desc1.MaxLuminance
                        << " nits"sv << std::endl
                        << "Max Full Luminance : "sv
                        << desc1.MaxFullFrameLuminance << " nits"sv;
    }

    // Capture format will be determined from the first call to
    // AcquireNextFrame()
    capture_format = DXGI_FORMAT_UNKNOWN;

    // Use CREATE_WAITABLE_TIMER_HIGH_RESOLUTION if supported (Windows 10 1809+)
    timer.reset(CreateWaitableTimerEx(nullptr, nullptr,
...
bool display_base_t::get_hdr_metadata(SS_HDR_METADATA &metadata) {
    ...
    // These are content-specific metadata parameters that this interface
    // doesn't give us
...
const char *format_str[] = {"DXGI_FORMAT_UNKNOWN",
...
std::vector<std::string> display_names(mem_type_e) {
    ...
    for (int x = 0; factory->EnumAdapters1(x, &adapter) != DXGI_ERROR_NOT_FOUND;
         ++x) {
        DXGI_ADAPTER_DESC1 adapter_desc;
        ...
        BOOST_LOG(debug) << std::endl
                         ...
                         << std::endl
                         << "Device Device ID : 0x"sv
                         << util::hex(adapter_desc.DeviceId).to_string_view()
                         << std::endl
                         << "Device Video Mem : "sv
                         << adapter_desc.DedicatedVideoMemory / 1048576
                         << " MiB"sv << std::endl
                         << "Device Sys Mem   : "sv
                         << adapter_desc.DedicatedSystemMemory / 1048576
                         << " MiB"sv << std::endl
                         << "Share Sys Mem    : "sv
                         << adapter_desc.SharedSystemMemory / 1048576
                         << " MiB"sv << std::endl
                         << std::endl
                         << "    ====== OUTPUT ======"sv << std::endl;
        ...
        dxgi::output_t::pointer output_p{};
        for (int y = 0;
             adapter->EnumOutputs(y, &output_p) != DXGI_ERROR_NOT_FOUND; ++y) {
            ...
            BOOST_LOG(debug)
                ...
                << "    AttachedToDesktop : "sv
                << (desc.AttachedToDesktop ? "yes"sv : "no"sv) << std::endl
                << "    Resolution        : "sv << width << 'x' << height
                << std::endl
                << std::endl;

            // Don't include the display in the list if we can't actually
            // capture it
            if (desc.AttachedToDesktop &&
                dxgi::test_dxgi_duplication(adapter, output)) {
