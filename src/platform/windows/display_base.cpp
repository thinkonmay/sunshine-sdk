...
int duplication_t::init(display_base_t *display, const ::video::config_t &config) {
  ...
  // FIXME: Duplicate output on RX580 in combination with DOOM (2016) --> BSOD
  {
    ...
    if (SUCCEEDED(status)) {
      ...
      for (int x = 0; x < 2; ++x) {
        ...
        status =
            output5->DuplicateOutput1((IUnknown *)display->device.get(), 0,
                                      supported_formats.size(), supported_formats.data(), &dup);
        if (SUCCEEDED(status)) {
          break;
        }
  ...
  BOOST_LOG(info) << "Desktop resolution ["sv << dup_desc.ModeDesc.Width << 'x'
                  << dup_desc.ModeDesc.Height << ']';
  BOOST_LOG(info) << "Desktop format ["sv
                  << display->dxgi_format_to_string(dup_desc.ModeDesc.Format) << ']';

  display->display_refresh_rate = dup_desc.ModeDesc.RefreshRate;
  double display_refresh_rate_decimal = 0.0;
...
capture_e duplication_t::next_frame(DXGI_OUTDUPL_FRAME_INFO &frame_info,
                                    std::chrono::milliseconds timeout, resource_t::pointer *res_p) {
  ...
  case S_OK:
    ...
    if (frame_info.ProtectedContentMaskedOut &&
        std::chrono::steady_clock::now() > last_protected_content_warning_time + 10s) {
      BOOST_LOG(warning)
...
capture_e display_base_t::capture(const push_captured_image_cb_t &push_captured_image_cb,
  ...
  auto clear_display_required = util::fail_guard([]() { SetThreadExecutionState(ES_CONTINUOUS); });
...
bool set_gpu_preference_on_self(int preference) {
  ...
  auto status =
      RegSetKeyValueW(HKEY_CURRENT_USER, L"Software\\Microsoft\\DirectX\\UserGpuPreferences",
                      sunshine_path, REG_SZ, value_data, (wcslen(value_data) + 1) * sizeof(WCHAR));
...
bool test_dxgi_duplication(adapter_t &adapter, output_t &output, bool enumeration_only) {
  ...
  device_t device;
  auto status =
      D3D11CreateDevice(adapter.get(), D3D_DRIVER_TYPE_UNKNOWN, nullptr, D3D11_CREATE_DEVICE_FLAGS,
...
int display_base_t::init(const ::video::config_t &config, const std::string &display_name) {
  ...
  std::call_once(windows_cpp_once_flag, []() {
    ...
    auto f = (User32_SetProcessDpiAwarenessContext)GetProcAddress(user32,
  ...
  status = D3D11CreateDevice(adapter_p, D3D_DRIVER_TYPE_UNKNOWN, nullptr, D3D11_CREATE_DEVICE_FLAGS,
                             featureLevels, sizeof(featureLevels) / sizeof(D3D_FEATURE_LEVEL),
  ...
  BOOST_LOG(debug) << std::endl
                   ...
                   << "Device Video Mem   : "sv << adapter_desc.DedicatedVideoMemory / 1048576
                   << " MiB"sv << std::endl
                   << "Device Sys Mem     : "sv << adapter_desc.DedicatedSystemMemory / 1048576
                   << " MiB"sv << std::endl
                   << "Share Sys Mem      : "sv << adapter_desc.SharedSystemMemory / 1048576
                   << " MiB"sv << std::endl
  ...
  // Bump up thread priority
  {
    ...
    if (gdi32) {
      auto check_hags = [&](const LUID &adapter) -> bool {
        auto d3dkmt_open_adapter =
            (PD3DKMTOpenAdapterFromLuid)GetProcAddress(gdi32, "D3DKMTOpenAdapterFromLuid");
        auto d3dkmt_query_adapter_info =
            (PD3DKMTQueryAdapterInfo)GetProcAddress(gdi32, "D3DKMTQueryAdapterInfo");
        auto d3dkmt_close_adapter =
            (PD3DKMTCloseAdapter)GetProcAddress(gdi32, "D3DKMTCloseAdapter");
        if (!d3dkmt_open_adapter || !d3dkmt_query_adapter_info || !d3dkmt_close_adapter) {
          BOOST_LOG(error)
              << "Couldn't load d3dkmt functions from gdi32.dll to determine GPU HAGS status";
          return false;
        }
        ...
        if (FAILED(d3dkmt_open_adapter(&d3dkmt_adapter))) {
          BOOST_LOG(error)
              << "D3DKMTOpenAdapterFromLuid() failed while trying to determine GPU HAGS status";
          return false;
        }
        ...
        if (SUCCEEDED(d3dkmt_query_adapter_info(&d3dkmt_adapter_info))) {
          ...
          BOOST_LOG(warning)
              << "D3DKMTQueryAdapterInfo() failed while trying to determine GPU HAGS status";
          result = false;
        }
        ...
        if (FAILED(d3dkmt_close_adapter(&d3dkmt_close_adapter_wrap))) {
          BOOST_LOG(error)
              << "D3DKMTCloseAdapter() failed while trying to determine GPU HAGS status";
        }

        return result;
      };

      auto d3dkmt_set_process_priority = (PD3DKMTSetProcessSchedulingPriorityClass)GetProcAddress(
      ...
      if (d3dkmt_set_process_priority) {
        ...
        if (adapter_desc.VendorId == 0x10DE) {
          // As of 2023.07, NVIDIA driver has unfixed bug(s) where "realtime" can cause
          // unrecoverable encoding freeze or outright driver crash This issue happens more
          // frequently with HAGS, in DX12 games or when VRAM is filled close to max capacity Track
          // OBS to see if they find better workaround or NVIDIA fixes it on their end, they seem to
          // be in communication
        ...
        BOOST_LOG(debug) << "Using "
                         << (priority == D3DKMT_SCHEDULINGPRIORITYCLASS_HIGH ? "high" : "realtime")
                         << " GPU priority";
        if (FAILED(d3dkmt_set_process_priority(GetCurrentProcess(), priority))) {
          BOOST_LOG(warning) << "Failed to adjust GPU priority. Please run application as "
                                "administrator for optimal performance.";
        }
      } else {
        BOOST_LOG(error) << "Couldn't load D3DKMTSetProcessSchedulingPriorityClass function from "
                            "gdi32.dll to adjust GPU priority";
      }
    }
    ...
    if (FAILED(status)) {
      BOOST_LOG(warning) << "Failed to increase capture GPU thread priority. Please run "
  ...
  // Try to reduce latency
  {
    ...
    status = device->QueryInterface(IID_IDXGIDevice, (void **)&dxgi);
    if (FAILED(status)) {
      BOOST_LOG(error) << "Failed to query DXGI interface from device [0x"sv
                       << util::hex(status).to_string_view() << ']';
      return -1;
    }
...
namespace platf {
std::shared_ptr<display_t> display(mem_type_e hwdevice_type, const std::string &display_name,
...
std::vector<std::string> display_names(mem_type_e) {
  ...
  for (int x = 0; factory->EnumAdapters1(x, &adapter) != DXGI_ERROR_NOT_FOUND; ++x) {
    ...
    for (int y = 0; adapter->EnumOutputs(y, &output_p) != DXGI_ERROR_NOT_FOUND; ++y) {
      ...
      BOOST_LOG(debug) << "    Output Name       : "sv << device_name << std::endl
