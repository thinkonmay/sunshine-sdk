...
void blend_cursor_monochrome(const cursor_t &cursor, img_t &img) {
  ...
  int delta_height =
      std::min(cursor_height - cursor_truncate_y, std::max(0, img.height - img_skip_y));
...
capture_e display_ddup_ram_t::snapshot(const pull_free_image_cb_t &pull_free_image_cb,
  ...
  if (frame_info.PointerShapeBufferSize > 0) {
    ...
    UINT dummy;
    status =
        dup.dup->GetFramePointerShape(img_data.size(), img_data.data(), &dummy, &cursor.shape_info);
    if (FAILED(status)) {
  ...
  if (frame_update_flag) {
    {
      ...
      // If we don't know the capture format yet, grab it from this texture and create the staging
      // texture
