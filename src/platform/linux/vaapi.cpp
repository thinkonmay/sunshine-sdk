...
// vaSyncBuffer stub allows Sunshine built against libva <2.9.0 to link against
// ffmpeg on libva 2.9.0 or later
...
struct DRMPRIMESurfaceDescriptor {
    ...
    struct {
        ...
        // Total size of this object (may include regions which are not part of
        // the surface)
        uint32_t size;
    ...
    struct {
        ...
        // references objects -->
        // DRMPRIMESurfaceDescriptor.objects[object_index[0]]
...
enum class entry_e {
     ...
     * VAEncFEIMBModeBufferType and VAEncFEIDistortionBufferType) for FEI entry
     * function. If separate PAK is set, two extra input buffers
     * (VAEncFEIMVBufferType, VAEncFEIMBModeBufferType) are needed for PAK
     * input.
     */
    FEI = 11,
    /**
     ...
     * the list of returned entry-points will include #Stats. Supported pixel
     * format, maximum resolution and statistics specific attributes can be
     * obtained via normal attribute query. One input buffer
...
typedef VAStatus (*queryConfigEntrypoints_fn)(VADisplay dpy, profile_e profile,
                                              entry_e *entrypoint_list,
                                              int *num_entrypoints);
...
typedef VAStatus (*initialize_fn)(VADisplay dpy, int *major_version,
                                  int *minor_version);
typedef const char *(*errorStr_fn)(VAStatus error_status);
typedef void (*VAMessageCallback)(void *user_context, const char *message);
typedef VAMessageCallback (*setErrorCallback_fn)(VADisplay dpy,
                                                 VAMessageCallback callback,
                                                 void *user_context);
typedef VAMessageCallback (*setInfoCallback_fn)(VADisplay dpy,
                                                VAMessageCallback callback,
                                                void *user_context);
typedef const char *(*queryVendorString_fn)(VADisplay dpy);
typedef VAStatus (*exportSurfaceHandle_fn)(VADisplay dpy,
   ...
   public:
    ...
    int set_frame(AVFrame *frame, AVBufferRef *hw_frames_ctx) override {
        ...
        auto status = va::exportSurfaceHandle(
            this->va_display, surface, va::SURFACE_ATTRIB_MEM_TYPE_DRM_PRIME_2,
            va::EXPORT_SURFACE_WRITE_ONLY | va::EXPORT_SURFACE_COMPOSED_LAYERS,
            &prime);
        if (status) {
            BOOST_LOG(error) << "Couldn't export va surface handle: ["sv
                             << (int)surface << "]: "sv << va::errorStr(status);
        ...
        auto sws_opt =
            egl::sws_t::make(width, height, frame->width, frame->height);
        if (!sws_opt) {
            return -1;
        }
    ...
    void apply_colorspace() override { sws.apply_colorspace(colorspace); }
...
class va_vram_t : public va_t {
   public:
    ...
    int init(int in_width, int in_height, file_t &&render_device, int offset_x,
             int offset_y) {
...
int vaapi_init_avcodec_hardware_input_buffer(
    platf::avcodec_encode_device_t *base, AVBufferRef **hw_device_buf) {
    if (!va::initialize) {
    ...
    if (!display) {
        ...
        BOOST_LOG(error)
            << "Couldn't open a va display from DRM with device: "sv
            << render_device;
        return -1;
    }
    ...
    if (status) {
        BOOST_LOG(error) << "Couldn't initialize va display: "sv
                         << va::errorStr(status);
        return -1;
    }

    BOOST_LOG(debug) << "vaapi vendor: "sv
                     << va::queryVendorString(display.get());

    *hw_device_buf = av_hwdevice_ctx_alloc(AV_HWDEVICE_TYPE_VAAPI);
    auto ctx = (AVHWDeviceContext *)(*hw_device_buf)->data;
    auto hwctx = (AVVAAPIDeviceContext *)ctx->hwctx;

    // Ownership of the VADisplay and DRM fd is now ours to manage via the
    // free() function
...
static bool query(display_t::pointer display, profile_e profile) {
    ...
    int count;
    auto status =
        queryConfigEntrypoints(display, profile, entrypoints.data(), &count);
    if (status) {
        BOOST_LOG(error) << "Couldn't query entrypoints: "sv
                         << va::errorStr(status);
        return false;
    }
...
bool validate(int fd) {
    ...
    if (!display) {
        ...
        auto bytes = readlink(("/proc/self/fd/" + std::to_string(fd)).c_str(),
    ...
    if (video::active_hevc_mode > 1 &&
        !query(display.get(), profile_e::HEVCMain)) {
        return false;
    }

    if (video::active_hevc_mode > 2 &&
        !query(display.get(), profile_e::HEVCMain10)) {
        return false;
    }
...
std::unique_ptr<platf::avcodec_encode_device_t> make_avcodec_encode_device(
    ...
    if (file.el < 0) {
        char string[1024];
        BOOST_LOG(error) << "Couldn't open "sv << render_device << ": "
                         << strerror_r(errno, string, sizeof(string));

        return nullptr;
    }

    return make_avcodec_encode_device(width, height, std::move(file), offset_x,
                                      offset_y, vram);
}
