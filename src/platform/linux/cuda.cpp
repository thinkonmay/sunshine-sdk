...
void pass_error(const std::string_view &sv, const char *name,
                const char *description) {
    BOOST_LOG(error) << sv << name << ':' << description;
}

void cff(CudaFunctions *cf) { cuda_free_functions(&cf); }
...
void freeStream(CUstream stream) {
    CU_CHECK_IGNORE(cdf->cuStreamDestroy(stream),
                    "Couldn't destroy cuda stream");
}
   ...
   public:
    ...
    int set_frame(AVFrame *frame, AVBufferRef *hw_frames_ctx) override {
        ...
        if (hwframe_ctx->sw_format != AV_PIX_FMT_NV12) {
            BOOST_LOG(error)
                << "cuda::cuda_t doesn't support any format other than AV_PIX_FMT_NV12"sv;
            return -1;
        }
        ...
        auto sws_opt =
            sws_t::make(width, height, frame->width, frame->height, width * 4);
        if (!sws_opt) {
            return -1;
        }
    ...
    void apply_colorspace() override {
        ...
        sws.convert(frame->data[0], frame->data[1], frame->linesize[0],
                    frame->linesize[1], tex->texture.linear, stream.get(),
    ...
    // When height and width don't change, it's not necessary to use linear
    // interpolation
   ...
   public:
    int convert(platf::img_t &img) override {
        return sws.load_ram(img, tex.array) ||
               sws.convert(frame->data[0], frame->data[1], frame->linesize[0],
                           frame->linesize[1], tex_obj(tex), stream.get());
    }
...
class cuda_vram_t : public cuda_t {
   public:
    int convert(platf::img_t &img) override {
        return sws.convert(frame->data[0], frame->data[1], frame->linesize[0],
                           frame->linesize[1], tex_obj(((img_t *)&img)->tex),
                           stream.get());
    }
};
   ...
   public:
    ctx_t(NVFBC_SESSION_HANDLE handle) {
        ...
        if (func.nvFBCBindContext(handle, &params)) {
            BOOST_LOG(error)
                << "Couldn't bind NvFBC context to current thread: "
                << func.nvFBCGetLastErrorStr(handle);
        }
    ...
    ~ctx_t() {
        ...
        if (func.nvFBCReleaseContext(handle, &params)) {
            BOOST_LOG(error)
                << "Couldn't release NvFBC context from current thread: "
                << func.nvFBCGetLastErrorStr(handle);
        }
    }
...
class handle_t {
   ...
   public:
    ...
    static std::optional<handle_t> make() {
        ...
        if (status) {
            BOOST_LOG(error)
                << "Failed to create session: "sv << handle.last_error();
    ...
    const char *last_error() { return func.nvFBCGetLastErrorStr(handle); }
    ...
    std::optional<NVFBC_GET_STATUS_PARAMS> status() {
        ...
        if (status) {
            BOOST_LOG(error)
                << "Failed to get NvFBC status: "sv << last_error();
    ...
    int capture(NVFBC_CREATE_CAPTURE_SESSION_PARAMS &capture_params) {
        if (func.nvFBCCreateCaptureSession(handle, &capture_params)) {
            BOOST_LOG(error)
                << "Failed to start capture session: "sv << last_error();
            return -1;
        }
        ...
        if (func.nvFBCToCudaSetUp(handle, &setup_params)) {
            BOOST_LOG(error) << "Failed to setup cuda interop with nvFBC: "sv
                             << last_error();
            return -1;
        }
        return 0;
    ...
    int stop() {
        ...
        if (func.nvFBCDestroyCaptureSession(handle, &params)) {
            BOOST_LOG(error)
                << "Couldn't destroy capture session: "sv << last_error();
    ...
    int reset() {
        ...
        if (func.nvFBCDestroyHandle(handle, &params)) {
            BOOST_LOG(error) << "Couldn't destroy session handle: "sv
                             << func.nvFBCGetLastErrorStr(handle);
        }
...
class display_t : public platf::display_t {
   public:
    int init(const std::string_view &display_name,
        ...
        if (!display_name.empty()) {
            if (status_params->bXRandRAvailable) {
                ...
                if (monitor_nr < 0 ||
                    monitor_nr >= status_params->dwOutputNum) {
                    BOOST_LOG(warning)
                        << "Can't stream monitor ["sv << monitor_nr
                        << "], it needs to be between [0] and ["sv
                        << status_params->dwOutputNum - 1
                        << "], defaulting to virtual desktop"sv;
                } else {
                    streamedMonitor = monitor_nr;
                }
            } else {
                BOOST_LOG(warning)
                    << "XrandR not available, streaming entire virtual desktop"sv;
            }
        }

        delay = std::chrono::nanoseconds{1s} / config.framerate;

        capture_params = NVFBC_CREATE_CAPTURE_SESSION_PARAMS{
    ...
    platf::capture_e capture(
        ...
        {
            // We must create at least one texture on this thread before calling
            // NvFBCToCudaSetUp() Otherwise it fails with "Unable to register an
            // OpenGL buffer to a CUDA resource (result: 201)" message
        ...
        ctx_t ctx{handle.handle};
        auto fg = util::fail_guard([&]() { handle.reset(); });
    ...
    platf::capture_e reinit(bool cursor) {
        ...
        if (capture_params.bAllowDirectCapture) {
            ...
            // Direct Capture may fail the first few times, even if it's
            // possible
            for (int x = 0; x < 3; ++x) {
                if (auto status =
                        func.nvFBCToCudaGrabFrame(handle.handle, &grab)) {
                    if (status == NVFBC_ERR_MUST_RECREATE) {
                        return platf::capture_e::reinit;
                    }

                    BOOST_LOG(error) << "Couldn't capture nvFramebuffer: "sv
                                     << handle.last_error();
                ...
                BOOST_LOG(debug)
                    << "Direct capture failed attempt ["sv << x << ']';
            }

            if (!info.bDirectCapture) {
                BOOST_LOG(debug)
                    << "Direct capture failed, trying the extra copy method"sv;
    ...
    platf::capture_e snapshot(const pull_free_image_cb_t &pull_free_image_cb,
        ...
        if (auto status = func.nvFBCToCudaGrabFrame(handle.handle, &grab)) {
            ...
            BOOST_LOG(error)
                << "Couldn't capture nvFramebuffer: "sv << handle.last_error();
            return platf::capture_e::error;
        }
        ...
        if (img->tex.copy((std::uint8_t *)device_ptr, img->height,
                          img->row_pitch)) {
            return platf::capture_e::error;
        }
...
std::shared_ptr<display_t> nvfbc_display(mem_type_e hwdevice_type,
    ...
    if (hwdevice_type != mem_type_e::cuda) {
        BOOST_LOG(error)
            << "Could not initialize nvfbc display with the given hw device type"sv;
        return nullptr;
    }
...
std::vector<std::string> nvfbc_display_names() {
    ...
    if (!status_params->bIsCapturePossible) {
        BOOST_LOG(error)
            << "NVidia driver doesn't support NvFBC screencasting"sv;
    }

    BOOST_LOG(info) << "Found ["sv << status_params->dwOutputNum
                    << "] outputs"sv;
    BOOST_LOG(info) << "Virtual Desktop: "sv << status_params->screenSize.w
                    << 'x' << status_params->screenSize.h;
    BOOST_LOG(info) << "XrandR: "sv
                    << (status_params->bXRandRAvailable ? "available"sv
                                                        : "unavailable"sv);
    ...
    for (auto x = 0; x < status_params->dwOutputNum; ++x) {
        ...
        BOOST_LOG(info) << "  Resolution: "sv << output.trackedBox.w << 'x'
                        << output.trackedBox.h;
        BOOST_LOG(info) << "  Offset: "sv << output.trackedBox.x << 'x'
                        << output.trackedBox.y;
