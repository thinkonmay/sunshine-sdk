...
class cap_sys_admin {
   public:
    cap_sys_admin() {
        ...
        if (cap_set_flag(caps, CAP_EFFECTIVE, 1, &sys_admin, CAP_SET) ||
            cap_set_proc(caps)) {
...
class wrapper_fb {
   public:
    ...
    wrapper_fb(drmModeFB2 *fb2)
        ...
        modifier = (fb2->flags & DRM_MODE_FB_MODIFIERS)
                       ? fb2->modifier
...
using obj_prop_t =
    util::safe_ptr<drmModeObjectProperties, drmModeFreeObjectProperties>;
...
#define _CONVERT(x, y) \
    ...
    BOOST_LOG(error) << "Unknown Monitor connector type ["sv << string
                     << "]: Please report this to the GitHub issue tracker"sv;
    return DRM_MODE_CONNECTOR_Unknown;
}

class plane_it_t
    : public round_robin_util::it_wrap_t<plane_t::element_type, plane_it_t> {
   public:
    ...
    plane_it_t(int fd, std::uint32_t *end) : fd{fd}, plane_p{end}, end{end} {}
    ...
    void inc() {
        ...
        for (; plane_p != end; ++plane_p) {
            ...
            if (!plane) {
                BOOST_LOG(error)
                    << "Couldn't get drm plane ["sv << (end - plane_p)
                    << "]: "sv << strerror(errno);
                continue;
            }
    ...
    bool eq(const plane_it_t &other) const { return plane_p == other.plane_p; }

    plane_t::pointer get() { return plane.get(); }
...
class card_t {
   public:
    using connector_interal_t =
        util::safe_ptr<drmModeConnector, drmModeFreeConnector>;
    ...
    int init(const char *path) {
        ...
        if (fd.el < 0) {
            BOOST_LOG(error)
                << "Couldn't open: "sv << path << ": "sv << strerror(errno);
            return -1;
        }
        ...
        if (rendernode_path) {
            ...
            if (render_fd.el < 0) {
                BOOST_LOG(warning)
                    << "Couldn't open render node: "sv << rendernode_path
                    << ": "sv << strerror(errno);
        ...
        if (drmSetClientCap(fd.el, DRM_CLIENT_CAP_UNIVERSAL_PLANES, 1)) {
            BOOST_LOG(error)
                << "Couldn't expose some/all drm planes for card: "sv << path;
            return -1;
        }

        if (drmSetClientCap(fd.el, DRM_CLIENT_CAP_ATOMIC, 1)) {
            BOOST_LOG(warning)
                << "Couldn't expose some properties for card: "sv << path;
        }
    ...
    crtc_t crtc(std::uint32_t id) { return drmModeGetCrtc(fd.el, id); }

    encoder_t encoder(std::uint32_t id) { return drmModeGetEncoder(fd.el, id); }

    res_t res() { return drmModeGetResources(fd.el); }
    ...
    std::uint32_t get_panel_orientation(std::uint32_t plane_id) {
        ...
        BOOST_LOG(error) << "Failed to determine panel orientation, defaulting "
                            "to landscape.";
    ...
    std::vector<connector_t> monitors(conn_type_count_t &conn_type_count) {
        ...
        std::for_each_n(resources->connectors, resources->count_connectors,
                        [this, &conn_type_count, &monitors](std::uint32_t id) {
    ...
    file_t handleFD(std::uint32_t handle) {
        ...
        auto status =
            drmPrimeHandleToFD(fd.el, handle, 0 /* flags */, &fb_fd.el);
        if (status) {
            return {};
        }
    ...
    std::vector<std::pair<prop_t, std::uint64_t>> props(std::uint32_t id,
                                                        std::uint32_t type) {
        ...
        for (auto x = 0; x < obj_prop->count_props; ++x) {
            props.emplace_back(drmModeGetProperty(fd.el, obj_prop->props[x]),
                               obj_prop->prop_values[x]);
        }

        return props;
    }

    std::vector<std::pair<prop_t, std::uint64_t>> plane_props(
        std::uint32_t id) {
    ...
    std::vector<std::pair<prop_t, std::uint64_t>> connector_props(
        std::uint32_t id) {
    ...
    std::uint32_t count() { return plane_res->count_planes; }

    plane_it_t begin() const {
        return plane_it_t{fd.el, plane_res->planes,
                          plane_res->planes + plane_res->count_planes};
    }
...
std::map<std::uint32_t, monitor_t> map_crtc_to_monitor(
    const std::vector<connector_t> &connectors) {
...
void print(plane_t::pointer plane, fb_t::pointer fb, crtc_t::pointer crtc) {
    if (crtc) {
        ...
        BOOST_LOG(debug) << "crtc("sv << crtc->width << ", "sv << crtc->height
                         << ')';
        BOOST_LOG(debug) << "plane->possible_crtcs == "sv
                         << plane->possible_crtcs;
    }

    BOOST_LOG(debug) << "x("sv << plane->x << ") y("sv << plane->y
                     << ") crtc_x("sv << plane->crtc_x << ") crtc_y("sv
                     << plane->crtc_y << ") crtc_id("sv << plane->crtc_id
                     << ')';

    BOOST_LOG(debug) << "Resolution: "sv << fb->width << 'x' << fb->height
                     << ": Pitch: "sv << fb->pitches[0] << ": Offset: "sv
                     << fb->offsets[0];

    std::stringstream ss;

    ss << "Format ["sv;
    std::for_each_n(plane->formats, plane->count_formats - 1,
                    [&ss](auto format) { ss << util::view(format) << ", "sv; });
   ...
   public:
    ...
    int init(const std::string &display_name, const ::video::config_t &config) {
        ...
        for (auto &entry : fs::directory_iterator{card_dir}) {
            ...
            if (filestring.size() < 4 ||
                std::string_view{filestring}.substr(0, 4) != "card"sv) {
                continue;
            }
            ...
            for (auto plane = std::begin(card); plane != end; ++plane) {
                ...
                if (!fb) {
                    BOOST_LOG(error)
                        << "Couldn't get drm fb for plane ["sv << plane->fb_id
                        << "]: "sv << strerror(errno);
                    return -1;
                }
                ...
                if (!fb->handles[0]) {
                    BOOST_LOG(error)
                        << "Couldn't get handle for DRM Framebuffer ["sv
                        << plane->fb_id
                        << "]: Possibly not permitted: do [sudo setcap cap_sys_admin+p sunshine]"sv;
                    return -1;
                }
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    if (fb_fd.el < 0) {
                        BOOST_LOG(error)
                            << "Couldn't get primary file descriptor for Framebuffer ["sv
                            << fb->fb_id << "]: "sv << strerror(errno);
                        continue;
                    }
                }
                ...
                // We need to find the correct /dev/dri/card{nr} to correlate
                // the crtc_id with the monitor descriptor
                auto pos = std::find_if(std::begin(card_descriptors),
                                        std::end(card_descriptors),
                                        [&](card_descriptor_t &cd) {
                                            return cd.path == filestring;
                                        });

                if (pos == std::end(card_descriptors)) {
                    // This code path shouldn't happen, but it's there just in
                    // case. card_descriptors is part of the guesswork after
                    // all.
                    BOOST_LOG(error) << "Couldn't find ["sv << entry.path()
                                     << "]: This shouldn't have happened :/"sv;
                    return -1;
                }
                ...
                if (monitor != std::end(pos->crtc_to_monitor)) {
                    ...
                    switch (card.get_panel_orientation(plane->plane_id)) {
                        case DRM_MODE_ROTATE_270:
                            BOOST_LOG(debug)
                                << "Detected panel orientation at 90, swapping "
                                   "width and height.";
                        ...
                        case DRM_MODE_ROTATE_180:
                            BOOST_LOG(warning)
                                << "Panel orientation is unsupported, screen "
                ...
                // crtc_to_monitor is part of the guesswork after all.
                    ...
                    BOOST_LOG(warning)
                        << "Couldn't find crtc_id, this shouldn't have happened :\\"sv;
                    width = crct->width;
                    height = crct->height;
                    offset_x = crct->x;
                    offset_y = crct->y;
    ...
    break_loop:
        if (monitor != monitor_index) {
            BOOST_LOG(error)
                << "Couldn't find monitor ["sv << monitor_index << ']';
    ...
    inline capture_e refresh(file_t *file, egl::surface_descriptor_t *sd) {
        ...
        auto fb = card.fb(plane.get());
        if (!fb) {
            BOOST_LOG(error) << "Couldn't get drm fb for plane ["sv
                             << plane->fb_id << "]: "sv << strerror(errno);
            return capture_e::error;
        }

        if (!fb->handles[0]) {
            BOOST_LOG(error)
                << "Couldn't get handle for DRM Framebuffer ["sv << plane->fb_id
                << "]: Possibly not permitted: do [sudo setcap cap_sys_admin+p sunshine]"sv;
            return capture_e::error;
        }
        ...
        for (int y = 0; y < 4; ++y) {
            if (!fb->handles[y]) {
                // setting sd->fds[y] to a negative value indicates that
                // sd->offsets[y] and sd->pitches[y] are uninitialized and
                // contain invalid values.
                sd->fds[y] = -1;
                // It's not clear whether there could still be valid handles
                // left. So, continue anyway.
   ...
   public:
    ...
    capture_e snapshot(const pull_free_image_cb_t &pull_free_image_cb,
        ...
        gl::ctx.GetTextureSubImage(
            rgb->tex[0], 0, img_offset_x, img_offset_y, 0, width, height, 1,
            GL_BGRA, GL_UNSIGNED_BYTE, img_out->height * img_out->row_pitch,
            img_out->data);
...
class display_vram_t : public display_t {
   public:
    ...
    std::unique_ptr<avcodec_encode_device_t> make_avcodec_encode_device(
        pix_fmt_e pix_fmt) override {
        if (mem_type == mem_type_e::vaapi) {
            return va::make_avcodec_encode_device(
                width, height, dup(card.render_fd.el), img_offset_x,
                img_offset_y, true);
        }

        BOOST_LOG(error)
            << "Unsupported pixel format for egl::display_vram_t: "sv
            << platf::from_pix_fmt(pix_fmt);
        return nullptr;
    }
    ...
    int dummy_img(platf::img_t *img) override {
        ...
        std::shared_ptr<platf::img_t> img_out;
        return snapshot(pull_dummy_img_callback, img_out, 1s, false) !=
               platf::capture_e::ok;
    }
    ...
    int init(const std::string &display_name, const ::video::config_t &config) {
        ...
        if (!va::validate(card.render_fd.el)) {
            BOOST_LOG(warning)
                << "Monitor "sv << display_name
                << " doesn't support hardware encoding. Reverting back to GPU -> RAM -> GPU"sv;
            return -1;
        }
...
/**
 * On Wayland, it's not possible to determine the position of the monitor on the
 * desktop with KMS. Wayland does allow applications to query attached monitors
 * on the desktop, however, the naming scheme is not standardized across
 * implementations.
 *
 * As a result, correlating the KMS output to the wayland outputs is guess work
 * at best. But, it's necessary for absolute mouse coordinates to work.
 *
...
void correlate_to_wayland(std::vector<kms::card_descriptor_t> &cds) {
    ...
    for (auto &monitor : monitors) {
        ...
        if (index_begin == std::string_view::npos) {
            ...
            index = std::max<int64_t>(
                1, util::from_view(name.substr(index_begin + 1)));
        }

        auto type = kms::from_view(name.substr(0, index_begin));

        for (auto &card_descriptor : cds) {
            for (auto &[_, monitor_descriptor] :
                 card_descriptor.crtc_to_monitor) {
                if (monitor_descriptor.index == index &&
                    monitor_descriptor.type == type) {
                    monitor_descriptor.viewport.offset_x =
                        monitor->viewport.offset_x;
                    monitor_descriptor.viewport.offset_y =
                        monitor->viewport.offset_y;

                    // A sanity check, it's guesswork after all.
                    if (monitor_descriptor.viewport.width !=
                            monitor->viewport.width ||
                        monitor_descriptor.viewport.height !=
                            monitor->viewport.height) {
                        BOOST_LOG(warning)
                            << "Mismatch on expected Resolution compared to actual resolution: "sv
                            << monitor_descriptor.viewport.width << 'x'
                            << monitor_descriptor.viewport.height << " vs "sv
                            << monitor->viewport.width << 'x'
                            << monitor->viewport.height;
                    }
...
std::vector<std::string> kms_display_names() {
    ...
    if (!fs::exists("/dev/dri")) {
        BOOST_LOG(warning)
            << "Couldn't find /dev/dri, kmsgrab won't be enabled"sv;
        return {};
    }
    ...
    for (auto &entry : fs::directory_iterator{card_dir}) {
        ...
        auto crtc_to_monitor =
            kms::map_crtc_to_monitor(card.monitors(conn_type_count));
        ...
        for (auto plane = std::begin(card); plane != end; ++plane) {
            ...
            if (!crtc) {
                BOOST_LOG(error)
                    << "Couldn't get crtc info: "sv << strerror(errno);
                return {};
            }
            ...
            kms::env_width =
                std::max(kms::env_width, (int)(crtc->x + crtc->width));
            kms::env_height =
                std::max(kms::env_height, (int)(crtc->y + crtc->height));
    ...
    for (auto &card_descriptor : cds) {
        for (auto &[_, monitor_descriptor] : card_descriptor.crtc_to_monitor) {
            ...
            BOOST_LOG(debug)
                << "Resolution: "sv << monitor_descriptor.viewport.width << 'x'
                << monitor_descriptor.viewport.height;
            BOOST_LOG(debug)
                << "Offset: "sv << monitor_descriptor.viewport.offset_x << 'x'
                << monitor_descriptor.viewport.offset_y;

            kms::env_width = std::max(
                kms::env_width, (int)(monitor_descriptor.viewport.offset_x +
                                      monitor_descriptor.viewport.width));
            kms::env_height = std::max(
                kms::env_height, (int)(monitor_descriptor.viewport.offset_y +
                                       monitor_descriptor.viewport.height));
        }
    }

    BOOST_LOG(debug) << "Desktop resolution: "sv << kms::env_width << 'x'
                     << kms::env_height;
