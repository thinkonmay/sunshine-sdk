...
void drain_errors(const std::string_view &prefix) {
    ...
    while ((err = ctx.GetError()) != GL_NO_ERROR) {
        BOOST_LOG(error) << "GL: "sv << prefix << ": ["sv
...
tex_t tex_t::make(std::size_t count) {
    ...
    for (auto tex : textures) {
        ...
        gl::ctx.TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
                              GL_CLAMP_TO_EDGE);  // x
        gl::ctx.TexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
                              GL_CLAMP_TO_EDGE);  // y
...
void frame_buf_t::copy(int id, int texture, int offset_x, int offset_y,
                       int width, int height) {
    gl::ctx.BindFramebuffer(GL_FRAMEBUFFER, (*this)[id]);
    gl::ctx.ReadBuffer(GL_COLOR_ATTACHMENT0 + id);
    gl::ctx.BindTexture(GL_TEXTURE_2D, texture);
    gl::ctx.CopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, offset_x, offset_y, width,
                              height);
}
...
util::Either<shader_t, std::string> shader_t::compile(
...
GLuint shader_t::handle() const { return _shader.el; }

buffer_t buffer_t::make(util::buffer_t<GLint> &&offsets, const char *block,
    ...
    ctx.BufferData(GL_UNIFORM_BUFFER, data.size(),
                   (const std::uint8_t *)data.data(), GL_DYNAMIC_DRAW);

    return buffer;
}

GLuint buffer_t::handle() const { return _buffer.el; }

const char *buffer_t::block() const { return _block; }

void buffer_t::update(const std::string_view &view, std::size_t offset) {
    ctx.BindBuffer(GL_UNIFORM_BUFFER, handle());
    ctx.BufferSubData(GL_UNIFORM_BUFFER, offset, view.size(),
                      (const void *)view.data());
}

void buffer_t::update(std::string_view *members, std::size_t count,
                      std::size_t offset) {
    ...
    for (int x = 0; x < count; ++x) {
        ...
        std::copy_n((const std::uint8_t *)val.data(), val.size(),
                    &buffer[_offsets[x]]);
    }
...
util::Either<program_t, std::string> program_t::link(const shader_t &vert,
                                                     const shader_t &frag) {
    program_t program;
    ...
    // p_handle stores a copy of the program handle, since program will be moved
    // before the fail guard function is called.
...
std::optional<buffer_t> program_t::uniform(
    ...
    for (int x = 0; x < count; ++x) {
        if (indices[x] == GL_INVALID_INDEX) {
            ...
            BOOST_LOG(error) << "Couldn't find ["sv << block << '.'
                             << members[x].first << ']';
        }
    }
    ...
    ctx.GetActiveUniformsiv(handle(), count, indices, GL_UNIFORM_OFFSET,
                            offsets.begin());
    ...
    return buffer_t::make(
        std::move(offsets), block,
        std::string_view{(char *)buffer.begin(), buffer.size()});
}

GLuint program_t::handle() const { return _program.el; }
...
bool fail() { return eglGetError() != EGL_SUCCESS; }
...
display_t make_display(
    ...
    switch (native_display.index()) {
        ...
        case 2:
            ...
            break;
        default:
            BOOST_LOG(error) << "egl::make_display(): Index ["sv
                             << native_display.index() << "] not implemented"sv;
            return nullptr;
    }

    // native_display.left() equals native_display.right()
    display_t display =
        eglGetPlatformDisplay(egl_platform, native_display_p, nullptr);
    ...
    if (!eglInitialize(display.get(), &major, &minor)) {
        BOOST_LOG(error) << "Couldn't initialize EGL display: ["sv
                         << util::hex(eglGetError()).to_string_view() << ']';
        return nullptr;
    }
...
std::optional<ctx_t> make_ctx(display_t::pointer display) {
    constexpr int conf_attr[]{EGL_RENDERABLE_TYPE, EGL_OPENGL_BIT, EGL_NONE};

    int count;
    EGLConfig conf;
    if (!eglChooseConfig(display, conf_attr, &conf, 1, &count)) {
        BOOST_LOG(error) << "Couldn't set config attributes: ["sv
                         << util::hex(eglGetError()).to_string_view() << ']';
        return std::nullopt;
    }

    if (!eglBindAPI(EGL_OPENGL_API)) {
        BOOST_LOG(error) << "Couldn't bind API: ["sv
                         << util::hex(eglGetError()).to_string_view() << ']';
        return std::nullopt;
    }

    constexpr int attr[]{EGL_CONTEXT_CLIENT_VERSION, 3, EGL_NONE};

    ctx_t ctx{display, eglCreateContext(display, conf, EGL_NO_CONTEXT, attr)};
    if (fail()) {
        BOOST_LOG(error) << "Couldn't create EGL context: ["sv
                         << util::hex(eglGetError()).to_string_view() << ']';
        return std::nullopt;
    }
    ...
    BOOST_LOG(debug) << "GL: shader: "sv
                     << gl::ctx.GetString(GL_SHADING_LANGUAGE_VERSION);
...
std::optional<rgb_t> import_source(display_t::pointer egl_display,
    ...
    rgb_t rgb{egl_display,
              eglCreateImage(egl_display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT,
                             nullptr, attribs),
              gl::tex_t::make(1)};

    if (!rgb->xrgb8) {
        BOOST_LOG(error) << "Couldn't import RGB Image: "sv
                         << util::hex(eglGetError()).to_string_view();
...
std::optional<nv12_t> import_target(
    ...
    EGLAttrib img_attr_planes[2][13]{
        {EGL_LINUX_DRM_FOURCC_EXT, DRM_FORMAT_R8, EGL_WIDTH, r8.width,
         EGL_HEIGHT, r8.height, EGL_DMA_BUF_PLANE0_FD_EXT, r8.fds[0],
         EGL_DMA_BUF_PLANE0_OFFSET_EXT, r8.offsets[0],
         EGL_DMA_BUF_PLANE0_PITCH_EXT, r8.pitches[0], EGL_NONE},

        {EGL_LINUX_DRM_FOURCC_EXT, DRM_FORMAT_GR88, EGL_WIDTH, gr88.width,
         EGL_HEIGHT, gr88.height, EGL_DMA_BUF_PLANE0_FD_EXT, r8.fds[0],
         EGL_DMA_BUF_PLANE0_OFFSET_EXT, gr88.offsets[0],
         EGL_DMA_BUF_PLANE0_PITCH_EXT, gr88.pitches[0], EGL_NONE},
    };

    nv12_t nv12{
        egl_display,
        eglCreateImage(egl_display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT,
                       nullptr, img_attr_planes[0]),
        eglCreateImage(egl_display, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT,
                       nullptr, img_attr_planes[1]),
        gl::tex_t::make(2),
...
void sws_t::apply_colorspace(const video::sunshine_colorspace_t &colorspace) {
    ...
    std::string_view members[]{
        util::view(color_p->color_vec_y), util::view(color_p->color_vec_u),
        util::view(color_p->color_vec_v), util::view(color_p->range_y),
        util::view(color_p->range_uv),
    };

    color_matrix.update(members,
                        sizeof(members) / sizeof(decltype(members[0])));

    program[0].bind(color_matrix);
    program[1].bind(color_matrix);
}

std::optional<sws_t> sws_t::make(int in_width, int in_height, int out_width,
    ...
    auto scalar =
        std::fminf(out_width / (float)in_width, out_heigth / (float)in_height);
...

    {
        ...
        for (int x = 0; x < count; ++x) {
            ...
            compiled_source = gl::shader_t::compile(read_file(sources[x]),
                                                    shader_type[x % 2]);
            gl_drain_errors;

            if (compiled_source.has_right()) {
                BOOST_LOG(error)
                    << sources[x] << ": "sv << compiled_source.right();
                error_flag = true;
            }
        }
        ...
        auto program = gl::program_t::link(compiled_sources[3].left(),
                                           compiled_sources[4].left());
        ...
        program = gl::program_t::link(compiled_sources[1].left(),
                                      compiled_sources[0].left());
        if (program.has_right()) {
            BOOST_LOG(error) << "GL linker: "sv << program.right();
            return std::nullopt;
        }
        ...
        program = gl::program_t::link(compiled_sources[3].left(),
                                      compiled_sources[2].left());
        if (program.has_right()) {
            BOOST_LOG(error) << "GL linker: "sv << program.right();
            return std::nullopt;
        }
    ...
    auto loc_width_i =
        gl::ctx.GetUniformLocation(sws.program[1].handle(), "width_i");
    if (loc_width_i < 0) {
    ...
    auto color_p = video::color_vectors_from_colorspace(
    ...
    auto color_matrix = sws.program[0].uniform(
        "ColorMatrix", members, sizeof(members) / sizeof(decltype(members[0])));
    if (!color_matrix) {
        return std::nullopt;
    }
...
int sws_t::blank(gl::frame_buf_t &fb, int offsetX, int offsetY, int width,
                 int height) {
    auto f = [&]() {
...
void sws_t::load_ram(platf::img_t &img) {
    ...
    gl::ctx.TexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, img.width, img.height,
                          GL_BGRA, GL_UNSIGNED_BYTE, img.data);
}

void sws_t::load_vram(img_descriptor_t &img, int offset_x, int offset_y,
                      int texture) {
    // When only a sub-part of the image must be encoded...
    const bool copy = offset_x || offset_y || img.sd.width != in_width ||
                      img.sd.height != in_height;
    if (copy) {
        auto framebuf = gl::frame_buf_t::make(1);
        framebuf.bind(&texture, &texture + 1);

        loaded_texture = tex[0];
        framebuf.copy(0, loaded_texture, offset_x, offset_y, in_width,
                      in_height);
    } else {
    ...
    if (img.data) {
        ...
        if (serial != img.serial) {
            ...
            gl::ctx.TexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, img.width,
                               img.height, 0, GL_BGRA, GL_UNSIGNED_BYTE,
                               img.data);
        }
        ...
        if (status != GL_FRAMEBUFFER_COMPLETE) {
            BOOST_LOG(error)
                << "Pass Cursor: CheckFramebufferStatus() --> [0x"sv
...
int sws_t::convert(gl::frame_buf_t &fb) {
    ...
    for (int x = 0; x < sizeof(attachments) / sizeof(decltype(attachments[0]));
         ++x) {
        ...
        if (status != GL_FRAMEBUFFER_COMPLETE) {
            BOOST_LOG(error)
                << "Pass "sv << x << ": CheckFramebufferStatus() --> [0x"sv
                << util::hex(status).to_string_view() << ']';
            return -1;
        }
#endif
        ...
        gl::ctx.Viewport(offsetX / (x + 1), offsetY / (x + 1),
                         out_width / (x + 1), out_height / (x + 1));
        gl::ctx.DrawArrays(GL_TRIANGLES, 0, 3);
    }
