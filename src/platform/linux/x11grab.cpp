...
_FN(GetImage, XImage *,
    (Display * display, Drawable d, int x, int y, unsigned int width,
...
_FN(GetOutputInfo, XRROutputInfo *,
    (Display * dpy, XRRScreenResources *resources, RROutput output));
_FN(GetCrtcInfo, XRRCrtcInfo *,
...
_FN(shm_get_image_unchecked, xcb_shm_get_image_cookie_t,
    (xcb_connection_t * c, xcb_drawable_t drawable, int16_t x, int16_t y,
...
_FN(shm_attach, xcb_void_cookie_t,
    (xcb_connection_t * c, xcb_shm_seg_t shmseg, uint32_t shmid,
     uint8_t read_only));
...
int init_shm() {
    ...
    std::vector<std::tuple<dyn::apiproc *, const char *>> funcs{
        ...
        {(dyn::apiproc *)&shm_get_image_unchecked,
...
using output_info_t =
    util::dyn_safe_ptr<_XRROutputInfo, &x11::rr::FreeOutputInfo>;
using screen_res_t =
    util::dyn_safe_ptr<_XRRScreenResources, &x11::rr::FreeScreenResources>;

class shm_id_t {
   public:
    shm_id_t() : id{-1} {}
    shm_id_t(int id) : id{id} {}
    shm_id_t(shm_id_t &&other) noexcept : id(other.id) { other.id = -1; }
...
static void blend_cursor(Display *display, img_t &img, int offsetX,
                         int offsetY) {
        ...
        std::for_each(overlay_begin, overlay_end, [&](long pixel) {
            ...
            if (alpha == 255) {
                *pixels_begin = *pixel_p;
                ...
                colors_in[0] = colors_out[0] +
                               (colors_in[0] * (255 - alpha) + 255 / 2) / 255;
                colors_in[1] = colors_out[1] +
                               (colors_in[1] * (255 - alpha) + 255 / 2) / 255;
                colors_in[2] = colors_out[2] +
                               (colors_in[2] * (255 - alpha) + 255 / 2) / 255;
            }
            ++pixels_begin;
        });
    }
...
struct x11_attr_t : public display_t {
     ...
     * This way we can trigger reinitialization if the dimensions changed while
     * streaming
     */
    ...
    int init(const std::string &display_name, const ::video::config_t &config) {
        ...
        if (streamedMonitor != -1) {
            BOOST_LOG(info) << "Configuring selected monitor ("sv
                            << streamedMonitor << ") to stream"sv;
            screen_res_t screenr{
                x11::rr::GetScreenResources(xdisplay.get(), xwindow)};
            int output = screenr->noutput;

            output_info_t result;
            int monitor = 0;
            for (int x = 0; x < output; ++x) {
                output_info_t out_info{x11::rr::GetOutputInfo(
                    xdisplay.get(), screenr.get(), screenr->outputs[x])};
                if (out_info) {
            ...
            if (!result) {
                BOOST_LOG(error)
                    << "Could not stream display number ["sv << streamedMonitor
                    << "], there are only ["sv << monitor << "] displays."sv;
                return -1;
            }

            if (result->crtc) {
                crtc_info_t crt_info{x11::rr::GetCrtcInfo(
                    xdisplay.get(), screenr.get(), result->crtc)};
                BOOST_LOG(info)
                    << "Streaming display: "sv << result->name << " with res "sv
                    << crt_info->width << 'x' << crt_info->height
                    << " offset by "sv << crt_info->x << 'x' << crt_info->y;
                ...
                BOOST_LOG(warning)
                    << "Couldn't get requested display info, defaulting to recording entire virtual desktop"sv;
                width = xattr.width;
                height = xattr.height;
            }
        } else {
     ...
     */
    void refresh() {
        x11::GetWindowAttributes(xdisplay.get(), xwindow,
                                 &xattr);  // Update xattr's
    }
    ...
    capture_e snapshot(const pull_free_image_cb_t &pull_free_image_cb,
        ...
        if (xattr.width != env_width || xattr.height != env_height) {
            BOOST_LOG(warning)
                << "X dimensions changed in non-SHM mode, request reinit"sv;
        ...
        XImage *x_img{x11::GetImage(xdisplay.get(), xwindow, offset_x, offset_y,
...
struct shm_attr_t : public x11_attr_t {
    x11::xdisplay_t
        shm_xdisplay;  // Prevent race condition with x11_attr_t::xdisplay
    xcb_connect_t xcb;
    ...
    void delayed_refresh() {
        ...
        refresh_task_id =
            task_pool.pushDelayed(&shm_attr_t::delayed_refresh, 2s, this)
                .task_id;
    }
    ...
    capture_e snapshot(const pull_free_image_cb_t &pull_free_image_cb,
        ...
        if (xattr.width != env_width || xattr.height != env_height) {
            BOOST_LOG(warning)
                << "X dimensions changed in SHM mode, request reinit"sv;
            return capture_e::reinit;
        } else {
            auto img_cookie = xcb::shm_get_image_unchecked(
                xcb.get(), display->root, offset_x, offset_y, width, height, ~0,
                XCB_IMAGE_FORMAT_Z_PIXMAP, seg, 0);

            xcb_img_t img_reply{
                xcb::shm_get_image_reply(xcb.get(), img_cookie, nullptr)};
            if (!img_reply) {
    ...
    std::uint32_t frame_size() { return width * height * 4; }
};

std::shared_ptr<display_t> x11_display(platf::mem_type_e hwdevice_type,
    ...
    if (hwdevice_type != platf::mem_type_e::system &&
        ...
        BOOST_LOG(error)
            << "Could not initialize x11 display with the given hw device type"sv;
        return nullptr;
    }

    if (xcb::init_shm() || xcb::init() || x11::init() || x11::rr::init() ||
        x11::fix::init()) {
...
std::vector<std::string> x11_display_names() {
    ...
    for (int x = 0; x < output; ++x) {
        ...
        if (out_info) {
            BOOST_LOG(info)
                << "Detected monitor "sv << monitor << ": "sv << out_info->name
                << ", connected: "sv << (out_info->connection == RR_Connected);
            ++monitor;
        }
    }
...
void freeImage(XImage *p) { XDestroyImage(p); }
void freeX(XFixesCursorImage *p) { x11::Free(p); }
...
int load_x11() {
    // This will be called once only
    static int x11_status = window_system == window_system_e::NONE ||
...
void cursor_t::capture(egl::cursor_t &img) {
    ...
    if (img.serial != xcursor->cursor_serial) {
        ...
        std::transform(xcursor->pixels, xcursor->pixels + buf_size / 4,
                       (int *)img.buffer.data(),
                       [](long pixel) -> int { return pixel; });
    }
...
xdisplay_t make_display() { return OpenDisplay(nullptr); }

void freeDisplay(_XDisplay *xdisplay) { CloseDisplay(xdisplay); }
