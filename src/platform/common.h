...
struct nvenc_encode_device_t : encode_device_t {
  virtual bool init_encoder(const video::config_t &client_config,
...
public:
  /**
   * When display has a new image ready or a timeout occurs, this callback will be called with the
   * image. If a frame was captured, frame_captured will be true. If a timeout occurred, it will be
   * false.
   *
   * On Break Request -->
   *    Returns false
   ...
   * bool *cursor --> A pointer to the flag that indicates whether the cursor should be captured as
   * well
   *
   * Returns either:
   ...
   */
  virtual capture_e capture(const push_captured_image_cb_t &push_captured_image_cb,
...
public:
  ...
  virtual std::unique_ptr<mic_t> microphone(const std::uint8_t *mapping, int channels,
