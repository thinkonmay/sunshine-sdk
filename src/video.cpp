...
public:
  int convert(platf::img_t &img) override {
    ...
    bool requires_padding = (sw_frame->width != sws_output_frame->width ||
                             sw_frame->height != sws_output_frame->height);
    ...
    auto status =
        sws_scale_frame(sws.get(), requires_padding ? sws_output_frame.get() : sw_frame.get(),
                        sws_input_frame.get());
    if (status < 0) {
      char string[AV_ERROR_MAX_STRING_SIZE];
      BOOST_LOG(error) << "Couldn't scale frame: "sv
    ...
    if (requires_padding) {
      ...
      for (int plane = 0; plane < planes; plane++) {
        ...
        auto offset = ((offsetW >> shift_w) * fmt_desc->comp[plane].step) +
                      (offsetH >> shift_h) * sw_frame->linesize[plane];
    ...
    if (frame->hw_frames_ctx) {
      ...
      if (status < 0) {
        char string[AV_ERROR_MAX_STRING_SIZE];
        BOOST_LOG(error) << "Failed to transfer image data to hardware frame: "sv
                         << av_make_error_string(string, AV_ERROR_MAX_STRING_SIZE, status);
        return -1;
      }
    }
  ...
  void prefill() {
    ...
    ptrdiff_t linesize[4] = {frame->linesize[0], frame->linesize[1], frame->linesize[2],
                             frame->linesize[3]};
    av_image_fill_black(frame->data, linesize, (AVPixelFormat)frame->format, frame->color_range,
  ...
  int init(int in_width, int in_height, AVFrame *frame, AVPixelFormat format, bool hardware) {
    ...
    if (status < 0) {
      char string[AV_ERROR_MAX_STRING_SIZE];
      BOOST_LOG(error) << "Failed to set SWS options: "sv
                       << av_make_error_string(string, AV_ERROR_MAX_STRING_SIZE, status);
      return -1;
    }
    ...
    if (status < 0) {
      char string[AV_ERROR_MAX_STRING_SIZE];
      BOOST_LOG(error) << "Failed to initialize SWS: "sv
                       << av_make_error_string(string, AV_ERROR_MAX_STRING_SIZE, status);
      return -1;
    }
...
enum flag_e : uint32_t {
  ...
  SINGLE_SLICE_ONLY =
      1 << 4, ///< Never use multiple slices. Older intel iGPU's ruin it for everyone else
  ...
  ALWAYS_REPROBE = 1 << 9,  ///< This is an encoder of last resort and we want to aggressively probe
                            ///< for a better one
...
public:
  ...
  avcodec_encode_session_t(avcodec_ctx_t &&avcodec_ctx,
...
// Keep a reference counter to ensure the capture thread only runs when other threads have a
...
encoder_t nvenc{
    ...
    std::make_unique<encoder_platform_formats_nvenc>(
        platf::mem_type_e::dxgi, platf::pix_fmt_e::nv12, platf::pix_fmt_e::p010),
    {
        {}, // Common options
...
encoder_t quicksync{
    ...
    std::make_unique<encoder_platform_formats_avcodec>(
        ...
        AV_PIX_FMT_P010, AV_PIX_FMT_VUYX, AV_PIX_FMT_XV30, dxgi_init_avcodec_hardware_input_buffer),
    {
        // Common options
        {
...
encoder_t amdvce{
    "amdvce"sv,
    std::make_unique<encoder_platform_formats_avcodec>(
        AV_HWDEVICE_TYPE_D3D11VA, AV_HWDEVICE_TYPE_NONE, AV_PIX_FMT_D3D11, AV_PIX_FMT_NV12,
        AV_PIX_FMT_P010, AV_PIX_FMT_NONE, AV_PIX_FMT_NONE, dxgi_init_avcodec_hardware_input_buffer),
    {
        // Common options
        {
            {"filler_data"s, false},
    ...
    },
    {
        ...
        {}, // YUV444 HDR-specific options
        {
            // Fallback options
            {"usage"s,
             2 /* AMF_VIDEO_ENCODER_USAGE_LOW_LATENCY */}, // Workaround for
                                                           // https://github.com/GPUOpen-LibrariesAndSDKs/AMF/issues/410
        },
        "h264_amf"s,
    },
    0};
...
void reset_display(std::shared_ptr<platf::display_t> &disp, const platf::mem_type_e &type,
 ...
 */
void refresh_displays(platf::mem_type_e dev_type, std::vector<std::string> &display_names,
  ...
  if (!current_display_name.empty()) {
    ...
    BOOST_LOG(warning) << "Previous active display ["sv << current_display_name
                       << "] is no longer present"sv;
  } else {
    for (int x = 0; x < display_names.size(); ++x) {
...
void captureThread(std::shared_ptr<safe::queue_t<capture_ctx_t>> capture_ctx_queue,
  ...
  while (capture_ctx_queue->running()) {
    ...
    case platf::capture_e::reinit: {
      ...
      // Some classes of images contain references to the display --> display won't delete unless
      // img is deleted
      for (auto &img : imgs) {
        img.reset();
      }
      ...
      while (capture_ctx_queue->running()) {
        ...
        if (capture_ctxs.front().config->display.has_value() &&
          ...
          BOOST_LOG(info) << "Display " << capture_ctxs.front().config->display.value()
                          << " not found";
...
int encode_nvenc(int64_t frame_nr, nvenc_encode_session_t &session,
  ...
  auto packet = std::make_unique<packet_raw_generic>(std::move(encoded_frame.data),
                                                     encoded_frame.frame_index, encoded_frame.idr);
  packet->channel_data = channel_data;
...
make_avcodec_encode_session(platf::display_t *disp, const encoder_t &encoder,
                            const config_t &config, int width, int height,
                            std::unique_ptr<platf::avcodec_encode_device_t> encode_device) {
  auto platform_formats =
      dynamic_cast<const encoder_platform_formats_avcodec *>(encoder.platform_formats.get());
  if (!platform_formats) {
    return nullptr;
  }
  ...
  auto sw_fmt = (colorspace.bit_depth == 8 && config.chromaSamplingType == 0)
                    ? platform_formats->avcodec_pix_fmt_8bit
                : (colorspace.bit_depth == 8 && config.chromaSamplingType == 1)
                    ? platform_formats->avcodec_pix_fmt_yuv444_8bit
                : (colorspace.bit_depth == 10 && config.chromaSamplingType == 0)
                    ? platform_formats->avcodec_pix_fmt_10bit
                : (colorspace.bit_depth == 10 && config.chromaSamplingType == 1)
  ...
  for (int retries = 0; retries < 2; retries++) {
    ...
    case 0:
      ...
      ctx->profile = (config.chromaSamplingType == 1) ? FF_PROFILE_H264_HIGH_444_PREDICTIVE
    ...
    ctx->gop_size = encoder.flags & LIMITED_GOP_SIZE ? std::numeric_limits<std::int16_t>::max()
    ...
    if (hardware) {
      ...
      if (platform_formats->avcodec_derived_dev_type != AV_HWDEVICE_TYPE_NONE) {
        ...
        // TODO
        auto err = av_hwdevice_ctx_create_derived(&derived_context,
    ...
    auto handle_option = [&options](const encoder_t::option_t &option) {
      std::visit(
          util::overloaded{[&](int v) { av_dict_set_int(&options, option.name.c_str(), v, 0); },
    ...
    for (auto &option :
         (config.dynamicRange ? video_format.hdr_options : video_format.sdr_options)) {
      handle_option(option);
    }
    if (config.chromaSamplingType == 1) {
      for (auto &option :
           (config.dynamicRange ? video_format.hdr444_options : video_format.sdr444_options)) {
        handle_option(option);
      }
    }
    if (retries > 0) {
    ...
    if (auto status = avcodec_open2(ctx.get(), codec, &options)) {
      ...
      if (!video_format.fallback_options.empty() && retries == 0) {
        BOOST_LOG(info) << "Retrying with fallback configuration options for ["sv
                        << video_format.name << "] after error: "sv
                        << av_make_error_string(err_str, AV_ERROR_MAX_STRING_SIZE, status);

        continue;
      } else {
        BOOST_LOG(error) << "Could not open codec ["sv << video_format.name << "]: "sv
                         << av_make_error_string(err_str, AV_ERROR_MAX_STRING_SIZE, status);
...
make_nvenc_encode_session(const config_t &client_config,
...
make_encode_session(platf::display_t *disp, const encoder_t &encoder, const config_t &config,
                    int width, int height, std::unique_ptr<platf::encode_device_t> encode_device) {
  if (encode_device) {
    ...
    BOOST_LOG(debug) << "Color range: ["sv
                     << (encode_device->colorspace.full_range ? "JPEG"sv : "MPEG"sv) << ']';
  }

  if (dynamic_cast<platf::avcodec_encode_device_t *>(encode_device.get())) {
    auto avcodec_encode_device =
        boost::dynamic_pointer_cast<platf::avcodec_encode_device_t>(std::move(encode_device));
    ...
    auto nvenc_encode_device =
        boost::dynamic_pointer_cast<platf::nvenc_encode_device_t>(std::move(encode_device));
...
void encode_run(int &frame_nr, // Store progress of the frame number
                ...
                std::unique_ptr<platf::encode_device_t> encode_device, safe::signal_t &reinit_event,
                const encoder_t &encoder, void *channel_data) {
...
make_encode_device(platf::display_t &disp, const encoder_t &encoder, const config_t &config) {
  ...
  auto pix_fmt = (colorspace.bit_depth == 10) ? encoder.platform_formats->pix_fmt_10bit
...
int validate_config(std::shared_ptr<platf::display_t> disp, const encoder_t &encoder,
  ...
  if (config.videoFormat <= 1) {
    if (auto packet_avcodec = dynamic_cast<packet_raw_avcodec *>(packet.get())) {
      if (cbs::validate_sps(packet_avcodec->av_packet,
...
bool validate_encoder(encoder_t &encoder, bool expect_failure) {
  ...
  reset_display(disp, encoder.platform_formats->dev_type, config::video.output_name,
                &config_autoselect);
  if (!disp) {
    return false;
  }
  ...
  for (auto [validate_flag, encoder_flag] : packet_deficiencies) {
    encoder.h264[encoder_flag] =
        (max_ref_frames_h264 & validate_flag && autoselect_h264 & validate_flag);
  }
    ...
    if (disp->is_codec_supported(encoder.hevc.name, config_autoselect)) {
      ...
      for (auto [validate_flag, encoder_flag] : packet_deficiencies) {
        encoder.hevc[encoder_flag] =
            (max_ref_frames_hevc & validate_flag && autoselect_hevc & validate_flag);
      }
    ...
    if (disp->is_codec_supported(encoder.av1.name, config_autoselect)) {
      ...
      for (auto [validate_flag, encoder_flag] : packet_deficiencies) {
        encoder.av1[encoder_flag] =
            (max_ref_frames_av1 & validate_flag && autoselect_av1 & validate_flag);
      }
  ...
  for (auto &[flag, config] : configs) {
    ...
    encoder.h264[flag] =
        flag != encoder_t::DYNAMIC_RANGE && validate_config(disp, encoder, h264) >= 0;
...
int probe_encoders() {
  ...
  auto adjust_encoder_constraints = [&](encoder_t *encoder) {
    // If we can't satisfy both the encoder and codec requirement, prefer the encoder over codec
    // support
  ...
  // If we haven't found an encoder yet, but we want one with specific codec support, search for
  // that now.
  ...
  if (chosen_encoder == nullptr) {
    ...
    if (!config::video.adapter_name.empty() || !config::video.output_name.empty()) {
      BOOST_LOG(fatal)
          << "Please ensure your manually chosen GPU and monitor are connected and powered on."sv;
    } else {
  ...
  for (int x = 0; x < encoder_t::MAX_FLAGS; ++x) {
    ...
    BOOST_LOG(debug) << encoder_t::from_flag(flag)
                     << (encoder.h264[flag] ? ": supported"sv : ": unsupported"sv);
  }
  BOOST_LOG(debug) << "-------------------"sv;
  ...
  if (active_hevc_mode == 0) {
    active_hevc_mode =
        encoder.hevc[encoder_t::PASSED] ? (encoder.hevc[encoder_t::DYNAMIC_RANGE] ? 3 : 2) : 1;
  }

  if (active_av1_mode == 0) {
    active_av1_mode =
        encoder.av1[encoder_t::PASSED] ? (encoder.av1[encoder_t::DYNAMIC_RANGE] ? 3 : 2) : 1;
  }
...
cuda_init_avcodec_hardware_input_buffer(platf::avcodec_encode_device_t *encode_device) {
  ...
  auto status = av_hwdevice_ctx_create(&hw_device_buf, AV_HWDEVICE_TYPE_CUDA, nullptr, nullptr,
...
dxgi_init_avcodec_hardware_input_buffer(platf::avcodec_encode_device_t *encode_device) {
  ...
  if (err) {
    char err_str[AV_ERROR_MAX_STRING_SIZE]{0};
    BOOST_LOG(error) << "Failed to create FFMpeg hardware device context: "sv
                     << av_make_error_string(err_str, AV_ERROR_MAX_STRING_SIZE, err);
...
int start_capture_async(capture_thread_async_ctx_t &capture_thread_ctx) {
  ...
  capture_thread_ctx.capture_thread = std::thread{
      captureThread, capture_thread_ctx.capture_ctx_queue, std::ref(capture_thread_ctx.display_wp),
      std::ref(capture_thread_ctx.reinit_event), std::ref(*capture_thread_ctx.encoder_p)};
