...
void free_ctx(AVCodecContext *ctx) { avcodec_free_context(&ctx); }

void free_frame(AVFrame *frame) { av_frame_free(&frame); }

void free_buffer(AVBufferRef *ref) { av_buffer_unref(&ref); }
...
using img_event_t =
    std::shared_ptr<safe::event_t<std::shared_ptr<platf::img_t>>>;
...
util::Either<avcodec_buffer_t, int> dxgi_init_avcodec_hardware_input_buffer(
    platf::avcodec_encode_device_t *);
util::Either<avcodec_buffer_t, int> vaapi_init_avcodec_hardware_input_buffer(
    platf::avcodec_encode_device_t *);
util::Either<avcodec_buffer_t, int> cuda_init_avcodec_hardware_input_buffer(
    platf::avcodec_encode_device_t *);
util::Either<avcodec_buffer_t, int> vt_init_avcodec_hardware_input_buffer(
    platf::avcodec_encode_device_t *);
...
class avcodec_software_encode_device_t : public platf::avcodec_encode_device_t {
   public:
    int convert(platf::img_t &img) override {
        ...
        const int linesizes[2]{img.row_pitch, 0};
        ...
        int ret = sws_scale(sws.get(), (std::uint8_t *const *)&img.data,
                            linesizes, 0, img.height, data, sw_frame->linesize);
        if (ret <= 0) {
            BOOST_LOG(error)
                << "Couldn't convert image to required format and/or size"sv;

            return -1;
        }

        // If frame is not a software frame, it means we still need to transfer
        // from main memory to vram memory
        if (frame->hw_frames_ctx) {
            auto status = av_hwframe_transfer_data(frame, sw_frame.get(), 0);
            if (status < 0) {
                char string[AV_ERROR_MAX_STRING_SIZE];
                BOOST_LOG(error)
                    << "Failed to transfer image data to hardware frame: "sv
                    << av_make_error_string(string, AV_ERROR_MAX_STRING_SIZE,
                                            status);
                return -1;
            }
        }
    ...
    void apply_colorspace() override {
        auto avcodec_colorspace =
            avcodec_colorspace_from_sunshine_colorspace(colorspace);
        sws_setColorspaceDetails(
            sws.get(), sws_getCoefficients(SWS_CS_DEFAULT), 0,
    ...
    int prefill() {
        ...
        sws_t sws{sws_getContext(width, height, AV_PIX_FMT_BGR0, width, height,
                                 (AVPixelFormat)frame->format,
                                 SWS_LANCZOS | SWS_ACCURATE_RND, nullptr,
                                 nullptr, nullptr)};
        ...
        const int linesizes[2]{width, 0};

        av_frame_make_writable(frame);

        auto data = img.begin();
        int ret = sws_scale(sws.get(), (std::uint8_t *const *)&data, linesizes,
    ...
    int init(int in_width, int in_height, AVFrame *frame, AVPixelFormat format,
        ...
        auto scalar = std::fminf((float)out_width / in_width,
        ...
        sws.reset(sws_getContext(
            in_width, in_height, AV_PIX_FMT_BGR0, out_width, out_height, format,
...
enum flag_e : uint32_t {
    ...
    LIMITED_GOP_SIZE = 1 << 3,   // Some encoders don't like it when you have an
                                 // infinite GOP_SIZE. *cough* VAAPI *cough*
    SINGLE_SLICE_ONLY = 1 << 4,  // Never use multiple slices <-- Older intel
                                 // iGPU's ruin it for everyone else :P
    CBR_WITH_VBR = 1 << 5,       // Use a VBR rate control mode to simulate CBR
    RELAXED_COMPLIANCE =
        1 << 6,                // Use FF_COMPLIANCE_UNOFFICIAL compliance mode
...
struct encoder_platform_formats_avcodec : encoder_platform_formats_t {
    using init_buffer_function_t =
        std::function<util::Either<avcodec_buffer_t, int>(
            platf::avcodec_encode_device_t *)>;
    ...
    encoder_platform_formats_avcodec(
        ...
        const init_buffer_function_t
            &init_avcodec_hardware_input_buffer_function)
        : avcodec_base_dev_type{avcodec_base_dev_type},
          ...
          init_avcodec_hardware_input_buffer{
              init_avcodec_hardware_input_buffer_function} {
...
struct encoder_platform_formats_nvenc : encoder_platform_formats_t {
    encoder_platform_formats_nvenc(const platf::mem_type_e &dev_type,
...
struct encoder_t {
    ...
    enum flag_e {
        ...
        CBR,  // Some encoders don't support CBR, if not supported --> attempt
              // constant quantatication parameter instead
        DYNAMIC_RANGE,   // hdr
        VUI_PARAMETERS,  // AMD encoder with VAAPI doesn't add VUI parameters to
                         // SPS
        MAX_FLAGS
    };
...
#undef _CONVERT
    ...
    struct option_t {
        ...
        std::string name;
        std::variant<int, int *, std::optional<int> *, std::function<int()>,
...
class avcodec_encode_session_t : public encode_session_t {
   public:
    ...
    avcodec_encode_session_t(avcodec_encode_session_t &&other) noexcept =
        default;
    ~avcodec_encode_session_t() {
        // Order matters here because the context relies on the hwdevice still
        // being valid
        avcodec_ctx.reset();
        device.reset();
    }
...
class nvenc_encode_session_t : public encode_session_t {
   public:
    ...
    void request_idr_frame() override { force_idr = true; }

    void request_normal_frame() override { force_idr = false; }
...
// Keep a reference counter to ensure the capture thread only runs when other
// threads have a reference to the capture thread
...
static encoder_t nvenc{
    "nvenc"sv,
    std::make_unique<encoder_platform_formats_nvenc>(platf::mem_type_e::dxgi,
...
static encoder_t quicksync{
    ...
    std::make_unique<encoder_platform_formats_avcodec>(
        AV_HWDEVICE_TYPE_D3D11VA, AV_HWDEVICE_TYPE_QSV, AV_PIX_FMT_QSV,
...
static encoder_t software{
    ...
    std::make_unique<encoder_platform_formats_avcodec>(
        ...
        AV_PIX_FMT_YUV420P, AV_PIX_FMT_YUV420P10, nullptr),
    {
        // libsvtav1 takes different presets than libx264/libx265.
        // We set an infinite GOP length, use a low delay prediction structure,
        // force I frames to be key frames, and set max bitrate to default to
        // work around a FFmpeg bug with CBR mode.
        {
            {"svtav1-params"s,
             "keyint=-1:pred-struct=1:force-key-frames=1:mbr=0"s},
        ...
        // poor real-time encoding performance, we do not enable libsvtav1 by
        // default. It is only suitable for testing AV1 until the IDR frame
        // issue is fixed.
        "libsvtav1"s,
#else
        {},
    ...
    },
    {
        ...
        // kicked to the 2nd packet in the frame, breaking Moonlight's parsing
        // logic. It also looks like gop_size isn't passed on to x265, so we
        // have to set 'keyint=-1' in the parameters ourselves.
        {
            {"forced-idr"s, 1},
...
static const std::vector<encoder_t *> encoders{&nvenc,
#ifdef _WIN32
                                               &quicksync, &amdvce,
#endif
#ifdef __linux__
...
void reset_display(std::shared_ptr<platf::display_t> &disp,
                   const platf::mem_type_e &type,
                   const std::string &display_name, config_t *config) {
...
std::string validate_display_name(std::vector<std::string> display_names,
    ...
    if (drop) {
        BOOST_LOG(info) << "invalid display name " << display_name;
...
void captureThread(
    std::shared_ptr<safe::queue_t<capture_ctx_t>> capture_ctx_queue,
    sync_util::sync_t<std::weak_ptr<platf::display_t>> &display_wp,
    safe::signal_t &reinit_event, const encoder_t &encoder) {
    ...
    auto display_names =
        platf::display_names(encoder.platform_formats->dev_type);
    ...
    auto disp = platf::display(encoder.platform_formats->dev_type, display_name,
    ...
    auto trim_imgs = [&]() {
        ...
        // decide whether to trim allocated unused above the currently used
        // count based on last used timestamp and universal timeout
        size_t trim_target = used_count;
        for (size_t i = used_count; i < imgs_used_timestamps.size(); i++) {
            if (imgs_used_timestamps[i] &&
                now - *imgs_used_timestamps[i] < trim_timeot) {
                trim_target = i;
            }
        }
    ...
    auto pull_free_image_callback =
        [&](std::shared_ptr<platf::img_t> &img_out) -> bool {
        img_out.reset();
        while (capture_ctx_queue->running()) {
            ...
            for (auto it = imgs.begin(); it != imgs.end(); it++) {
                if (*it && it->use_count() == 1) {
                    img_out = *it;
                    if (it != imgs.begin()) {
                        // move image to the front of the list to prioritize its
                        // reusal
                        imgs.erase(it);
                ...
                for (auto it = imgs.begin(); it != imgs.end(); it++) {
                    if (!*it) {
                        ...
                        *it = disp->alloc_img();
                        img_out = *it;
                        if (it != imgs.begin()) {
                            // move image to the front of the list to prioritize
                            // its reusal
                            imgs.erase(it);
                            imgs.push_front(img_out);
                        }
            ...
            if (img_out) {
                // trim allocated but unused portion of the pool based on
                // timeouts
                trim_imgs();
    ...
    while (capture_ctx_queue->running()) {
                ...
                capture_ctx != std::end(capture_ctxs), {
                    if (!capture_ctx->images->running()) {
                        ...
                        display_name = validate_display_name(
                            display_names,
                            capture_ctx->switch_display->pop().value());
                        BOOST_LOG(info)
                            << "capturing screen on display " << display_name;
                        return false;
        ...
        auto status = disp->capture(push_captured_image_callback,
                                    pull_free_image_callback, &display_cursor);
        ...
        switch (status) {
            case platf::capture_e::reinit: {
                ...
                // Some classes of images contain references to the display -->
                // display won't delete unless img is deleted
                for (auto &img : imgs) {
                    img.reset();
                }
                ...
                while (display_wp->use_count() != 1) {
                    // Free images that weren't consumed by the encoders. These
                    // can reference the display and prevent the ref count from
                    // reaching 1. We do this here rather than on the encoder
                    // thread to avoid race conditions where the encoding loop
                    // might free a good frame after reinitializing if we
                    // capture a new frame here before the encoder has finished
                    // reinitializing.
                    KITTY_WHILE_LOOP(
                        auto capture_ctx = std::begin(capture_ctxs),
                        capture_ctx != std::end(capture_ctxs), {
                ...
                while (capture_ctx_queue->running()) {
                    ...
                    reset_display(disp, encoder.platform_formats->dev_type,
                                  display_name, capture_ctxs.front().config);
                    if (disp) {
                        break;
                    }
                }
            ...
            default:
                BOOST_LOG(error)
                    << "Unrecognized capture status ["sv << (int)status << ']';
                return;
        }
    }
}
...
int encode_avcodec(
    int64_t frame_nr, avcodec_encode_session_t &session,
    ...
    while (ret >= 0) {
        ...
        if ((frame->flags & AV_FRAME_FLAG_KEY) &&
            ...
            BOOST_LOG(error)
                << "Encoder did not produce IDR frame when requested!"sv;
        }
        ...
        if (session.inject) {
            if (session.inject == 1) {
                ...
                session.replacements.emplace_back(
                    std::string_view((char *)std::begin(vps.old),
                                     vps.old.size()),
                    std::string_view((char *)std::begin(vps._new),
                                     vps._new.size()));
            }
            ...
            session.replacements.emplace_back(
                ...
                std::string_view((char *)std::begin(sps._new),
                                 sps._new.size()));
        }
...
// IMPORTANT
int encode_nvenc(
    int64_t frame_nr, nvenc_encode_session_t &session,
    safe::mail_raw_t::queue_t<packet_t> &packets, void *channel_data,
    std::optional<std::chrono::steady_clock::time_point> frame_timestamp) {
    ...
    if (frame_nr != encoded_frame.frame_index) {
        BOOST_LOG(error) << "NvENC frame index mismatch " << frame_nr << " "
                         << encoded_frame.frame_index;
    }

    auto packet = std::make_unique<packet_raw_generic>(
        std::move(encoded_frame.data), encoded_frame.frame_index,
        encoded_frame.idr);
    packet->channel_data = channel_data;
    packet->after_ref_frame_invalidation =
        encoded_frame.after_ref_frame_invalidation;
    packet->frame_timestamp = frame_timestamp;
    packets->wait(MAX_QUEUE_SIZE);
    packets->raise(std::move(packet));
...
int encode(
    int64_t frame_nr, encode_session_t &session,
    safe::mail_raw_t::queue_t<packet_t> &packets, void *channel_data,
    std::optional<std::chrono::steady_clock::time_point> frame_timestamp) {
    if (auto avcodec_session =
            dynamic_cast<avcodec_encode_session_t *>(&session)) {
        return encode_avcodec(frame_nr, *avcodec_session, packets, channel_data,
                              frame_timestamp);
    } else if (auto nvenc_session =
                   dynamic_cast<nvenc_encode_session_t *>(&session)) {
        return encode_nvenc(frame_nr, *nvenc_session, packets, channel_data,
                            frame_timestamp);
    }
...
std::unique_ptr<avcodec_encode_session_t> make_avcodec_encode_session(
    platf::display_t *disp, const encoder_t &encoder, const config_t &config,
    int width, int height,
    std::unique_ptr<platf::avcodec_encode_device_t> encode_device) {
    auto platform_formats =
        dynamic_cast<const encoder_platform_formats_avcodec *>(
            encoder.platform_formats.get());
    if (!platform_formats) {
        return nullptr;
    }

    bool hardware =
        platform_formats->avcodec_base_dev_type != AV_HWDEVICE_TYPE_NONE;

    auto &video_format = config.videoFormat == 0   ? encoder.h264
                         : config.videoFormat == 1 ? encoder.hevc
                                                   : encoder.av1;
    if (!video_format[encoder_t::PASSED] ||
        !disp->is_codec_supported(video_format.name, config)) {
        BOOST_LOG(error) << encoder.name << ": "sv << video_format.name
                         << " mode not supported"sv;
        return nullptr;
    }

    if (config.dynamicRange && !video_format[encoder_t::DYNAMIC_RANGE]) {
        BOOST_LOG(error) << video_format.name
                         << ": dynamic range not supported"sv;
        return nullptr;
    }
    ...
    switch (config.videoFormat) {
        ...
        case 1:
            ctx->profile = config.dynamicRange ? FF_PROFILE_HEVC_MAIN_10
                                               : FF_PROFILE_HEVC_MAIN;
            break;

        case 2:
            // AV1 supports both 8 and 10 bit encoding with the same Main
            // profile
    ...
    if (config.numRefFrames) {
        if (video_format[encoder_t::REF_FRAMES_RESTRICT]) {
            ...
            BOOST_LOG(warning)
                << "Client requested reference frame limit, but encoder doesn't support it!"sv;
        }
    }
    ...
    auto sw_fmt = (colorspace.bit_depth == 10)
    ...
    if (hardware) {
        ...
        auto buf_or_error =
            platform_formats->init_avcodec_hardware_input_buffer(
                encode_device.get());
        ...
        // If this encoder requires derivation from the base, derive the desired
        // type
        if (platform_formats->avcodec_derived_dev_type !=
            AV_HWDEVICE_TYPE_NONE) {
            avcodec_buffer_t derived_context;

            // Allow the hwdevice to prepare for this type of context to be
            // derived
            if (encode_device->prepare_to_derive_context(
                    platform_formats->avcodec_derived_dev_type)) {
                return nullptr;
            }

            auto err = av_hwdevice_ctx_create_derived(
                &derived_context, platform_formats->avcodec_derived_dev_type,
                encoding_stream_context.get(), 0);
            if (err) {
                char err_str[AV_ERROR_MAX_STRING_SIZE]{0};
                BOOST_LOG(error) << "Failed to derive device context: "sv
                                 << av_make_error_string(
                                        err_str, AV_ERROR_MAX_STRING_SIZE, err);
        ...
        {
            avcodec_buffer_t frame_ref{
                av_hwframe_ctx_alloc(encoding_stream_context.get())};
        ...
        ctx->slices =
            std::max(config.slicesPerFrame, config::video.min_threads);
    }
    ...
    auto handle_option = [&options](const encoder_t::option_t &option) {
        std::visit(
            util::overloaded{
                ...
                [&](std::string *v) {
                    if (!v->empty())
                        av_dict_set(&options, option.name.c_str(), v->c_str(),
                                    0);
                }},
            option.value);
    };
    ...
    for (auto &option : (config.dynamicRange ? video_format.hdr_options
                                             : video_format.sdr_options)) {
        handle_option(option);
    }
    ...
    if (video_format[encoder_t::CBR]) {
        ...
        if (!(encoder.flags & NO_RC_BUF_LIMIT)) {
            if (!hardware && (ctx->slices > 1 || config.videoFormat == 1)) {
                // Use a larger rc_buffer_size for software encoding when slices
                // are enabled, because libx264 can severely degrade quality if
                // the buffer is too small. libx265 encounters this issue more
                // frequently, so always scale the buffer by 1.5x for software
                // HEVC encoding.
        ...
        BOOST_LOG(error) << "Couldn't set video quality: encoder "sv
                         << encoder.name << " doesn't support qp"sv;
        return nullptr;
    }

    if (auto status = avcodec_open2(ctx.get(), codec, &options)) {
        char err_str[AV_ERROR_MAX_STRING_SIZE]{0};
        BOOST_LOG(error) << "Could not open codec ["sv << video_format.name
                         << "]: "sv
                         << av_make_error_string(
                                err_str, AV_ERROR_MAX_STRING_SIZE, status);
    ...
    if (colorspace_is_hdr(colorspace)) {
        ...
        if (disp->get_hdr_metadata(hdr_metadata)) {
            auto mdm =
                av_mastering_display_metadata_create_side_data(frame.get());

            mdm->display_primaries[0][0] =
                av_make_q(hdr_metadata.displayPrimaries[0].x, 50000);
            mdm->display_primaries[0][1] =
                av_make_q(hdr_metadata.displayPrimaries[0].y, 50000);
            mdm->display_primaries[1][0] =
                av_make_q(hdr_metadata.displayPrimaries[1].x, 50000);
            mdm->display_primaries[1][1] =
                av_make_q(hdr_metadata.displayPrimaries[1].y, 50000);
            mdm->display_primaries[2][0] =
                av_make_q(hdr_metadata.displayPrimaries[2].x, 50000);
            mdm->display_primaries[2][1] =
                av_make_q(hdr_metadata.displayPrimaries[2].y, 50000);

            mdm->white_point[0] = av_make_q(hdr_metadata.whitePoint.x, 50000);
            mdm->white_point[1] = av_make_q(hdr_metadata.whitePoint.y, 50000);

            mdm->min_luminance =
                av_make_q(hdr_metadata.minDisplayLuminance, 10000);
            mdm->max_luminance = av_make_q(hdr_metadata.maxDisplayLuminance, 1);

            mdm->has_luminance = hdr_metadata.maxDisplayLuminance != 0 ? 1 : 0;
            mdm->has_primaries =
                hdr_metadata.displayPrimaries[0].x != 0 ? 1 : 0;

            if (hdr_metadata.maxContentLightLevel != 0 ||
                hdr_metadata.maxFrameAverageLightLevel != 0) {
                auto clm =
                    av_content_light_metadata_create_side_data(frame.get());
            ...
            BOOST_LOG(error)
                << "Couldn't get display hdr metadata when colorspace "
                   "selection indicates it should have one";
        }
    }
    ...
    if (!encode_device->data) {
        auto software_encode_device =
            std::make_unique<avcodec_software_encode_device_t>();

        if (software_encode_device->init(width, height, frame.get(), sw_fmt,
                                         hardware)) {
            return nullptr;
        }
    ...
    auto session = std::make_unique<avcodec_encode_session_t>(
        std::move(ctx), std::move(encode_device_final),

        // 0 ==> don't inject, 1 ==> inject for h264, 2 ==> inject for hevc
        config.videoFormat <= 1
            ? (1 - (int)video_format[encoder_t::VUI_PARAMETERS]) *
...
std::unique_ptr<nvenc_encode_session_t> make_nvenc_encode_session(
    const config_t &client_config,
    std::unique_ptr<platf::nvenc_encode_device_t> encode_device) {
    if (!encode_device->init_encoder(client_config,
    ...
    if (encode_device) {
        switch (encode_device->colorspace.colorspace) {
            case colorspace_e::bt2020:
                BOOST_LOG(info)
                    << "HDR color coding [Rec. 2020 + SMPTE 2084 PQ]"sv;
                break;
        ...
        BOOST_LOG(info) << "Color depth: "
                        << encode_device->colorspace.bit_depth << "-bit";
        BOOST_LOG(info) << "Color range: ["sv
                        << (encode_device->colorspace.full_range ? "JPEG"sv
                                                                 : "MPEG"sv)
                        << ']';
    }

    if (dynamic_cast<platf::avcodec_encode_device_t *>(encode_device.get())) {
        auto avcodec_encode_device =
            dynamic_pointer_cast<platf::avcodec_encode_device_t>(
                std::move(encode_device));
        return make_avcodec_encode_session(disp, encoder, config, width, height,
                                           std::move(avcodec_encode_device));
    } else if (dynamic_cast<platf::nvenc_encode_device_t *>(
                   encode_device.get())) {
        auto nvenc_encode_device =
            dynamic_pointer_cast<platf::nvenc_encode_device_t>(
                std::move(encode_device));
        return make_nvenc_encode_session(config,
                                         std::move(nvenc_encode_device));
    }

    return nullptr;
}

void encode_run(int &frame_nr,  // Store progress of the frame number
                safe::mail_t mail, img_event_t images, config_t *config,
                std::shared_ptr<platf::display_t> disp,
                std::unique_ptr<platf::encode_device_t> encode_device,
                safe::signal_t &reinit_event, const encoder_t &encoder,
                void *channel_data) {
    auto session =
        make_encode_session(disp.get(), encoder, *config, disp->width,
                            disp->height, std::move(encode_device));
    if (!session) {
        return;
    }
...

    {
        ...
        // encode even if we timeout waiting on the first frame. This is a
        // relatively large allocation which can be freed immediately after
        // convert(), so we do this in a separate scope.
        auto dummy_img = disp->alloc_img();
        if (!dummy_img || disp->dummy_img(dummy_img.get()) ||
            session->convert(*dummy_img)) {
            return;
        }
    }
...
std::unique_ptr<platf::encode_device_t> make_encode_device(
    ...
    auto pix_fmt = (colorspace.bit_depth == 10)
                       ? encoder.platform_formats->pix_fmt_10bit
                       : encoder.platform_formats->pix_fmt_8bit;

    if (dynamic_cast<const encoder_platform_formats_avcodec *>(
            encoder.platform_formats.get())) {
        result = disp.make_avcodec_encode_device(pix_fmt);
    } else if (dynamic_cast<const encoder_platform_formats_nvenc *>(
                   encoder.platform_formats.get())) {
...
void capture_async(safe::mail_t mail, config_t &config, void *channel_data) {
    ...
    ref->capture_ctx_queue->raise(
        capture_ctx_t{mail->event<std::string>(mail::switch_display),
                      mail->event<bool>(mail::toggle_cursor), images, &config});
    ...
    while (!stopped && images->running()) {
        if (mail->event<bool>(mail::shutdown)->peek()) break;
        // Wait for the main capture event when the display is being
        // reinitialized
        ...
        encode_run(frame_nr, mail, images, &config, display,
                   std::move(encode_device), ref->reinit_event, *ref->encoder_p,
                   channel_data);
    }
}
...
int validate_config(std::shared_ptr<platf::display_t> &disp,
                    const encoder_t &encoder, config_t config) {
    reset_display(disp, encoder.platform_formats->dev_type,
                  config::video.output_name, &config);
    if (!disp) {
        return -1;
    }
    ...
    auto session = make_encode_session(disp.get(), encoder, config, disp->width,
                                       disp->height, std::move(encode_device));
    if (!session) {
        return -1;
    }
    ...
    {
        // Image buffers are large, so we use a separate scope to free it
        // immediately after convert()
    ...
    if (config.videoFormat <= 1) {
        if (auto packet_avcodec =
                dynamic_cast<packet_raw_avcodec *>(packet.get())) {
            if (cbs::validate_sps(
...
bool validate_encoder(encoder_t &encoder, bool expect_failure) {
    ...
    auto test_hevc = active_hevc_mode >= 2 ||
                     (active_hevc_mode == 0 && !(encoder.flags & H264_ONLY));
    auto test_av1 = active_av1_mode >= 2 ||
                    (active_av1_mode == 0 && !(encoder.flags & H264_ONLY));
    ...
    reset_display(disp, encoder.platform_formats->dev_type,
                  config::video.output_name, &config_autoselect);
    if (!disp) {
        return false;
    }
    if (!disp->is_codec_supported(encoder.h264.name, config_autoselect)) {
        ...
        BOOST_LOG(info) << "Encoder ["sv << encoder.name
                        << "] is not supported on this GPU"sv;
        return false;
    }

retry:
    // If we're expecting failure, use the autoselect ref config first since
    // that will always succeed if the encoder is available.
    auto max_ref_frames_h264 =
        expect_failure ? -1
                       : validate_config(disp, encoder, config_max_ref_frames);
    auto autoselect_h264 =
        max_ref_frames_h264 >= 0
            ? max_ref_frames_h264
            : validate_config(disp, encoder, config_autoselect);
    if (autoselect_h264 < 0) {
        if (encoder.h264.qp && encoder.h264[encoder_t::CBR]) {
            // It's possible the encoder isn't accepting Constant Bit Rate. Turn
            // off CBR and make another attempt
            encoder.h264.capabilities.set();
        ...
        // We expected failure, but actually succeeded. Do the max_ref_frames
        // probe we skipped.
        max_ref_frames_h264 =
            validate_config(disp, encoder, config_max_ref_frames);
    }

    std::vector<std::pair<validate_flag_e, encoder_t::flag_e>>
        packet_deficiencies{
            {VUI_PARAMS, encoder_t::VUI_PARAMETERS},
        };

    for (auto [validate_flag, encoder_flag] : packet_deficiencies) {
        encoder.h264[encoder_flag] = (max_ref_frames_h264 & validate_flag &&
                                      autoselect_h264 & validate_flag);
    }
    ...
    if (test_hevc) {
        ...
        retry_hevc:
            auto max_ref_frames_hevc =
                validate_config(disp, encoder, config_max_ref_frames);
            auto autoselect_hevc =
                max_ref_frames_hevc >= 0
                    ? max_ref_frames_hevc
                    : validate_config(disp, encoder, config_autoselect);
            if (autoselect_hevc < 0 && encoder.hevc.qp &&
                encoder.hevc[encoder_t::CBR]) {
            ...
            for (auto [validate_flag, encoder_flag] : packet_deficiencies) {
                encoder.hevc[encoder_flag] =
                    (max_ref_frames_hevc & validate_flag &&
                     autoselect_hevc & validate_flag);
            }

            encoder.hevc[encoder_t::REF_FRAMES_RESTRICT] =
                max_ref_frames_hevc >= 0;
            encoder.hevc[encoder_t::PASSED] =
                max_ref_frames_hevc >= 0 || autoselect_hevc >= 0;
        } else {
            BOOST_LOG(info) << "Encoder ["sv << encoder.hevc.name
                            << "] is not supported on this GPU"sv;
    ...
    if (test_av1) {
        ...
        retry_av1:
            auto max_ref_frames_av1 =
                validate_config(disp, encoder, config_max_ref_frames);
            auto autoselect_av1 =
                max_ref_frames_av1 >= 0
                    ? max_ref_frames_av1
                    : validate_config(disp, encoder, config_autoselect);
            if (autoselect_av1 < 0 && encoder.av1.qp &&
                encoder.av1[encoder_t::CBR]) {
                // It's possible the encoder isn't accepting Constant Bit Rate.
                // Turn off CBR and make another attempt
                encoder.av1.capabilities.set();
            ...
            for (auto [validate_flag, encoder_flag] : packet_deficiencies) {
                encoder.av1[encoder_flag] =
                    (max_ref_frames_av1 & validate_flag &&
                     autoselect_av1 & validate_flag);
            }

            encoder.av1[encoder_t::REF_FRAMES_RESTRICT] =
                max_ref_frames_av1 >= 0;
            encoder.av1[encoder_t::PASSED] =
                max_ref_frames_av1 >= 0 || autoselect_av1 >= 0;
        } else {
            BOOST_LOG(info) << "Encoder ["sv << encoder.av1.name
                            << "] is not supported on this GPU"sv;
    ...
    for (auto &[flag, config] : configs) {
        ...
        encoder.h264[flag] = flag != encoder_t::DYNAMIC_RANGE &&
                             validate_config(disp, encoder, h264) >= 0;
    ...
    encoder.hevc[encoder_t::VUI_PARAMETERS] =
        encoder.hevc[encoder_t::VUI_PARAMETERS] &&
        !config::sunshine.flags[config::flag::FORCE_VIDEO_HEADER_REPLACE];

    if (!encoder.h264[encoder_t::VUI_PARAMETERS]) {
        BOOST_LOG(warning) << encoder.name
                           << ": h264 missing sps->vui parameters"sv;
    }
    if (encoder.hevc[encoder_t::PASSED] &&
        !encoder.hevc[encoder_t::VUI_PARAMETERS]) {
        BOOST_LOG(warning) << encoder.name
                           << ": hevc missing sps->vui parameters"sv;
    }
...
int probe_encoders() {
    ...
    auto adjust_encoder_constraints = [&](encoder_t *encoder) {
        // If we can't satisfy both the encoder and codec requirement, prefer
        // the encoder over codec support
        if (active_hevc_mode == 3 && !encoder->hevc[encoder_t::DYNAMIC_RANGE]) {
            BOOST_LOG(warning)
                << "Encoder ["sv << encoder->name
                << "] does not support HEVC Main10 on this system"sv;
            active_hevc_mode = 0;
        } else if (active_hevc_mode == 2 && !encoder->hevc[encoder_t::PASSED]) {
            BOOST_LOG(warning) << "Encoder ["sv << encoder->name
                               << "] does not support HEVC on this system"sv;
            active_hevc_mode = 0;
        }
        ...
        if (active_av1_mode == 3 && !encoder->av1[encoder_t::DYNAMIC_RANGE]) {
            BOOST_LOG(warning)
                << "Encoder ["sv << encoder->name
                << "] does not support AV1 Main10 on this system"sv;
            active_av1_mode = 0;
        } else if (active_av1_mode == 2 && !encoder->av1[encoder_t::PASSED]) {
            BOOST_LOG(warning) << "Encoder ["sv << encoder->name
                               << "] does not support AV1 on this system"sv;
            active_av1_mode = 0;
        }
    };

    if (!config::video.encoder.empty()) {
        // If there is a specific encoder specified, use it if it passes
        // validation
        KITTY_WHILE_LOOP(
            auto pos = std::begin(encoder_list), pos != std::end(encoder_list),
            {
                auto encoder = *pos;

                if (encoder->name == config::video.encoder) {
                    // Remove the encoder from the list entirely if it fails
                    // validation
                    if (!validate_encoder(
                            *encoder,
                    ...
                    // We will return an encoder here even if it fails one of
                    // the codec requirements specified by the user
        ...
        if (chosen_encoder == nullptr) {
            BOOST_LOG(error) << "Couldn't find any working encoder matching ["sv
                             << config::video.encoder << ']';
        }
    }

    BOOST_LOG(info)
        << "// Testing for available encoders, this may generate errors. You can safely ignore those errors. //"sv;

    // If we haven't found an encoder yet, but we want one with specific codec
    // support, search for that now.
    if (chosen_encoder == nullptr &&
        ...
        if (chosen_encoder == nullptr) {
            BOOST_LOG(error)
                << "Couldn't find any working encoder that meets HEVC/AV1 requirements"sv;
        }
    }

    // If no encoder was specified or the specified encoder was unusable, keep
    // trying the remaining encoders until we find one that passes validation.
    if (chosen_encoder == nullptr) {
        KITTY_WHILE_LOOP(
            auto pos = std::begin(encoder_list), pos != std::end(encoder_list),
            {
                ...
                // If we've used a previous encoder and it's not this one, we
                // expect this encoder to fail to validate. It will use a
                // slightly different order of checks to more quickly eliminate
                // failing encoders.
                if (!validate_encoder(
                        *encoder,
                        previous_encoder && previous_encoder != encoder)) {
                ...
                // We will return an encoder here even if it fails one of the
                // codec requirements specified by the user
                adjust_encoder_constraints(encoder);
    ...
    BOOST_LOG(info);
    BOOST_LOG(info)
        << "// Ignore any errors mentioned above, they are not relevant. //"sv;
    BOOST_LOG(info);

    auto &encoder = *chosen_encoder;

    last_encoder_probe_supported_ref_frames_invalidation =
        (encoder.flags & REF_FRAMES_INVALIDATION);

    BOOST_LOG(debug) << "------  h264 ------"sv;
    for (int x = 0; x < encoder_t::MAX_FLAGS; ++x) {
        auto flag = (encoder_t::flag_e)x;
        BOOST_LOG(debug) << encoder_t::from_flag(flag)
                         << (encoder.h264[flag] ? ": supported"sv
                                                : ": unsupported"sv);
    }
    BOOST_LOG(debug) << "-------------------"sv;
    BOOST_LOG(info) << "Found H.264 encoder: "sv << encoder.h264.name << " ["sv
                    << encoder.name << ']';
    ...
    if (encoder.hevc[encoder_t::PASSED]) {
        ...
        for (int x = 0; x < encoder_t::MAX_FLAGS; ++x) {
            auto flag = (encoder_t::flag_e)x;
            BOOST_LOG(debug)
                << encoder_t::from_flag(flag)
                << (encoder.hevc[flag] ? ": supported"sv : ": unsupported"sv);
        }
        BOOST_LOG(debug) << "-------------------"sv;

        BOOST_LOG(info) << "Found HEVC encoder: "sv << encoder.hevc.name
                        << " ["sv << encoder.name << ']';
    }
    ...
    if (encoder.av1[encoder_t::PASSED]) {
        ...
        for (int x = 0; x < encoder_t::MAX_FLAGS; ++x) {
            ...
            BOOST_LOG(debug)
                << encoder_t::from_flag(flag)
                << (encoder.av1[flag] ? ": supported"sv : ": unsupported"sv);
        }
        BOOST_LOG(debug) << "-------------------"sv;

        BOOST_LOG(info) << "Found AV1 encoder: "sv << encoder.av1.name << " ["sv
                        << encoder.name << ']';
    }
    ...
    if (active_hevc_mode == 0) {
        active_hevc_mode =
            encoder.hevc[encoder_t::PASSED]
                ? (encoder.hevc[encoder_t::DYNAMIC_RANGE] ? 3 : 2)
                : 1;
    }

    if (active_av1_mode == 0) {
        active_av1_mode = encoder.av1[encoder_t::PASSED]
                              ? (encoder.av1[encoder_t::DYNAMIC_RANGE] ? 3 : 2)
                              : 1;
    }
...
util::Either<avcodec_buffer_t, int> vaapi_init_avcodec_hardware_input_buffer(
    ...
    // If an egl hwdevice
    if (encode_device->data) {
        if (((vaapi_init_avcodec_hardware_input_buffer_fn)encode_device->data)(
                encode_device, &hw_device_buf)) {
            return -1;
        }
    ...
    auto render_device = config::video.adapter_name.empty()
                             ? nullptr
                             : config::video.adapter_name.c_str();

    auto status = av_hwdevice_ctx_create(&hw_device_buf, AV_HWDEVICE_TYPE_VAAPI,
                                         render_device, nullptr, 0);
    if (status < 0) {
        char string[AV_ERROR_MAX_STRING_SIZE];
        BOOST_LOG(error) << "Failed to create a VAAPI device: "sv
                         << av_make_error_string(
                                string, AV_ERROR_MAX_STRING_SIZE, status);
        return -1;
    }
...
util::Either<avcodec_buffer_t, int> cuda_init_avcodec_hardware_input_buffer(
    ...
    auto status =
        av_hwdevice_ctx_create(&hw_device_buf, AV_HWDEVICE_TYPE_CUDA, nullptr,
                               nullptr, 1 /* AV_CUDA_USE_PRIMARY_CONTEXT */);
    if (status < 0) {
        char string[AV_ERROR_MAX_STRING_SIZE];
        BOOST_LOG(error) << "Failed to create a CUDA device: "sv
                         << av_make_error_string(
                                string, AV_ERROR_MAX_STRING_SIZE, status);
        return -1;
    }
...
util::Either<avcodec_buffer_t, int> vt_init_avcodec_hardware_input_buffer(
    ...
    auto status = av_hwdevice_ctx_create(
        &hw_device_buf, AV_HWDEVICE_TYPE_VIDEOTOOLBOX, nullptr, nullptr, 0);
    if (status < 0) {
        char string[AV_ERROR_MAX_STRING_SIZE];
...
util::Either<avcodec_buffer_t, int> dxgi_init_avcodec_hardware_input_buffer(
    ...
    auto ctx =
        (AVD3D11VADeviceContext *)((AVHWDeviceContext *)ctx_buf->data)->hwctx;
    ...
    if (err) {
        char err_str[AV_ERROR_MAX_STRING_SIZE]{0};
        BOOST_LOG(error)
            << "Failed to create FFMpeg hardware device context: "sv
...
int start_capture_async(capture_thread_async_ctx_t &capture_thread_ctx) {
    ...
    capture_thread_ctx.capture_thread =
        std::thread{captureThread, capture_thread_ctx.capture_ctx_queue,
