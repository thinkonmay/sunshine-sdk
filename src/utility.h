...
static constexpr auto instantiation_of_v =
    __instantiation_of<X, T, Y...>::value;
...
class Hex {
   ...
   public:
    ...
    std::string to_string() const { return {begin(), end()}; }
...
T from_hex(const std::string_view &hex, bool rev = false) {
    ...
    auto buf_size =
        std::count_if(std::begin(hex), std::end(hex), is_convertable) / 2;
   ...
   public:
    ...
    std::size_t operator()(const value_type &value) const {
        ...
        return std::hash<std::string_view>{}(
            std::string_view{p, sizeof(value_type)});
    }
};
...
class Either : public std::variant<std::monostate, X, Y> {
   public:
    ...
    constexpr bool has_left() const { return std::holds_alternative<X>(*this); }
    constexpr bool has_right() const {
        return std::holds_alternative<Y>(*this);
    }

    X &left() { return std::get<X>(*this); }

    Y &right() { return std::get<Y>(*this); }

    const X &left() const { return std::get<X>(*this); }

    const Y &right() const { return std::get<Y>(*this); }
};

// Compared to std::unique_ptr, it adds the ability to get the address of the
// pointer itself
...
class uniq_ptr {
   public:
    ...
    uniq_ptr(V *p) noexcept : _p{p} {
        static_assert(std::is_same_v<element_type, void> ||
                          std::is_same_v<element_type, V> ||
    ...
    std::add_lvalue_reference_t<element_type> operator*() { return *_p; }
...
shared_t<P> make_shared(T *pointer) {
    return shared_t<P>(reinterpret_cast<typename P::pointer>(pointer),
...
class wrap_ptr {
   public:
    ...
    wrap_ptr &operator=(std::unique_ptr<V> &&uniq_ptr) {
        static_assert(std::is_base_of_v<element_type, V>,
                      "element_type must be base class of V");
        _own_ptr = true;
        _p = uniq_ptr.release();
...
constexpr bool is_pointer_v =
    ...
    instantiation_of_v<std::shared_ptr, T> || instantiation_of_v<uniq_ptr, T> ||
    std::is_pointer_v<T>;
...
template <class T>
using optional_t =
    either_t<(std::is_same_v<T, bool> || is_pointer_v<T>), T, std::optional<T>>;
...
class buffer_t {
   public:
    ...
    T &operator[](size_t el) { return _buf[el]; }

    const T &operator[](size_t el) const { return _buf[el]; }

    size_t size() const { return _els; }

    void fake_resize(std::size_t els) { _els = els; }
...
template <class T, class ReturnType,
          typename Function<ReturnType, T>::type function>
struct Destroy {
    typedef T pointer;

    void operator()(pointer p) { function(p); }
};
...
template <class T>
struct endian_helper<
    T, std::enable_if_t<!(instantiation_of_v<std::optional, T>)>> {
...
template <class T>
struct endian_helper<T,
                     std::enable_if_t<instantiation_of_v<std::optional, T>>> {
    static inline T little(T x) {
        if (!x) return x;
