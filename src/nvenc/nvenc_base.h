...
public:
  ...
  bool create_encoder(const nvenc_config &config, const video::config_t &client_config,
                      const nvenc_colorspace_t &colorspace, NV_ENC_BUFFER_FORMAT buffer_format);
...
protected:
  ...
  /**
   * @brief This function returns the corresponding struct version for the minimum API required by
   * the codec.
   ...
   */
  uint32_t min_struct_version(uint32_t version, uint32_t v11_struct_version = 0,
