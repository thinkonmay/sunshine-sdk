...
bool nvenc_base::create_encoder(const nvenc_config &config, const video::config_t &client_config,
  ...
  minimum_api_version =
      (client_config.videoFormat <= 1) ? MAKE_NVENC_VER(11U, 0U) : MAKE_NVENC_VER(12U, 0U);
  ...
  if (nvenc_failed(nvenc->nvEncGetEncodeGUIDs(encoder, encode_guids.data(), encode_guids.size(),
                                              &encode_guid_count))) {
...

  {
    ...
    if (std::find_if(encode_guids.begin(), encode_guids.end(), search_predicate) ==
        encode_guids.end()) {
...

  {
    ...
    if (encoder_params.width > supported_width || encoder_params.height > supported_height) {
      BOOST_LOG(error) << "NvEnc: gpu max encode resolution " << supported_width << "x"
                       << supported_height << ", requested " << encoder_params.width << "x"
  ...
  NV_ENC_PRESET_CONFIG preset_config = {min_struct_version(NV_ENC_PRESET_CONFIG_VER),
                                        {min_struct_version(NV_ENC_CONFIG_VER, 7, 8)}};
  if (nvenc_failed(nvenc->nvEncGetEncodePresetConfigEx(encoder, init_params.encodeGUID,
  ...
  if (get_encoder_cap(NV_ENC_CAPS_SUPPORT_CUSTOM_VBV_BUF_SIZE)) {
    ...
    if (config.vbv_percentage_increase > 0) {
      enc_config.rcParams.vbvBufferSize +=
          enc_config.rcParams.vbvBufferSize * config.vbv_percentage_increase / 100;
    }
  }
  ...
  auto set_ref_frames = [&](uint32_t &ref_frames_option, NV_ENC_NUM_REF_FRAMES &L0_option,
    ...
    // This limits ref frames any frame can use to 1, but allows larger buffer size for fallback if
    // some frames are invalidated through rfi
  ...
  case 0: {
    ...
    enc_config.profileGUID =
        buffer_is_yuv444() ? NV_ENC_H264_PROFILE_HIGH_444_GUID : NV_ENC_H264_PROFILE_HIGH_GUID;
  ...
  case 2: {
    ...
    if (client_config.slicesPerFrame > 1) {
      ...
      format_config.numTileRows =
          std::pow(2, std::ceil(std::log2(client_config.slicesPerFrame) / 2));
      format_config.numTileColumns =
          std::pow(2, std::floor(std::log2(client_config.slicesPerFrame) / 2));
    }
    break;
  }
  }
  ...
  NV_ENC_CREATE_BITSTREAM_BUFFER create_bitstream_buffer = {
      min_struct_version(NV_ENC_CREATE_BITSTREAM_BUFFER_VER)};
...

  {
    ...
    if (enc_config.rcParams.multiPass != NV_ENC_MULTI_PASS_DISABLED)
      extra += " two-pass";
    if (config.vbv_percentage_increase > 0 &&
        get_encoder_cap(NV_ENC_CAPS_SUPPORT_CUSTOM_VBV_BUF_SIZE))
...
nvenc_encoded_frame nvenc_base::encode_frame(uint64_t frame_index, bool force_idr) {
  ...
  auto unmap_guard =
      util::fail_guard([&] { nvenc->nvEncUnmapInputResource(encoder, &mapped_input_buffer); });
...
bool nvenc_base::invalidate_ref_frames(uint64_t first_frame, uint64_t last_frame) {
  ...
  if (last_frame < first_frame) {
    BOOST_LOG(error) << "NvEnc: invaid rfi request " << first_frame << "-" << last_frame
                     << ", generating IDR";
    return false;
  }
...
/**
 * @brief This function returns the corresponding struct version for the minimum API required by the
 * codec.
 * @details Reducing the struct versions maximizes driver compatibility by avoiding needless API
 * breaks.
 * @param version The raw structure version from `NVENCAPI_STRUCT_VERSION()`.
 * @param v11_struct_version Optionally specifies the struct version to use with v11 SDK major
 * versions.
 * @param v12_struct_version Optionally specifies the struct version to use with v12 SDK major
 * versions.
 * @return A suitable struct version for the active codec.
 */
uint32_t nvenc_base::min_struct_version(uint32_t version, uint32_t v11_struct_version,
                                        uint32_t v12_struct_version) {
  ...
  if (v11_struct_version || v12_struct_version) {
    version &= ~(0xFFu << 16);
    version |= (((minimum_api_version & 0xFF) >= 12) ? v12_struct_version : v11_struct_version)
               << 16;
  }
